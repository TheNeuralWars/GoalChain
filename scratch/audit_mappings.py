...
def audit():
    ...
    # Files actually in folders
    folder_files = set(os.listdir(PLAYERS_FOLDER))
    print(f"📁 Total files in {PLAYERS_FOLDER}: {len(folder_files)}")
    ...
    for item in first_mapping:
        ...
        exists = filename in folder_files
        print(f"[{'✔️' if exists else '❌'}] {filename} -> ID {p_id}: {name} ({real_name})")
        if exists:
            first_ok += 1
    print(f"First mapping coverage: {first_ok}/{len(first_mapping)} files present.")

    print("\n--- SECOND MAPPING AUDIT ---")
    second_ok = 0
    for filename, p_id in second_mapping.items():
        ...
        if exists:
            second_ok += 1
    print(f"Second mapping coverage: {second_ok}/{len(second_mapping)} files present.")

...
function runGitScore(windowCommits) {
    ...
    for (const line of lines) {
        ...
        if (line.startsWith("@@@")) {
            ...
            if (currentEmail && !scores[currentEmail])
                scores[currentEmail] = 0;
            if (currentEmail)
                scores[currentEmail] += 20; // commit base weight
            continue;
        }
        ...
        if (Number.isFinite(add))
            scores[currentEmail] += add;
        if (Number.isFinite(del))
            scores[currentEmail] += Math.floor(del / 2);
    }
    return scores;
}
function loadWalletMap(filePath) {
...
async function main() {
    ...
    const oracle = new OracleService(rpcUrl, keypairPath, process.env.PROGRAM_ID || "FbDhM4itBS2Cco7c7PbNvC98Fx7Y5HxqXS1JuXdNcBwg");
    ...
    const [builderFundPda] = PublicKey.findProgramAddressSync([Buffer.from("builder_fund"), oracle.configPda.toBuffer()], oracle.program.programId);
    const [contributorVault] = PublicKey.findProgramAddressSync([Buffer.from("builder_vault"), builderFundPda.toBuffer(), Buffer.from("contributors")], oracle.program.programId);
    ...
    const builderFundState = await oracle.program.account.builderFund.fetch(builderFundPda);

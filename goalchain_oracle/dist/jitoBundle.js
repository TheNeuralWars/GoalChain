...
(function (PriorityTier) {
    PriorityTier["ECONOMY"] = "economy";
    PriorityTier["STANDARD"] = "standard";
    PriorityTier["PRIORITY"] = "priority";
 ...
 */
export async function fetchTipAccounts(blockEngineUrl) {
 ...
 */
export function selectTipAccount(tipAccounts) {
 ...
 */
export function calculateJitoTip(basePriorityFeeMicroLamports, tier) {
 ...
 */
export function createTipInstruction(payer, tipAccount, tipLamports) {
 ...
 */
export async function submitBundle(blockEngineUrl, transactions) {
    const rpcUrl = `${blockEngineUrl}/api/v1/bundles`;
    // Serialize transactions to base64
    const encodedTransactions = transactions.map((tx) => Buffer.from(tx.serialize()).toString("base64"));
    const payload = {
        jsonrpc: "2.0",
        id: 1,
        method: "sendBundle",
 ...
 */
export async function waitForBundleConfirmation(connection, bundleId, blockEngineUrl, options = {}) {
    const { maxAttempts = 30, pollIntervalMs = 1000, commitment = "confirmed", } = options;
 ...
 */
export async function simulateBundle(blockEngineUrl, transactions) {
...
export async function buildVaultCrankBundle(params) {
 ...
 */
export async function executeVaultCrankBundle(connection, payer, stakingIx, buybackTx, burnTx, options = {}) {
    const { blockEngineUrl = BLOCK_ENGINE_URLS.devnet, priorityTier = PriorityTier.STANDARD, maxPollAttempts = 30, } = options;

...
(function (MarketType) {
    MarketType["MatchResultLive"] = "MatchResultLive";
    MarketType["NextGoal"] = "NextGoal";
...
(function (MatchResult) {
    MatchResult["TeamA"] = "TeamA";
    MatchResult["TeamB"] = "TeamB";
    MatchResult["Draw"] = "Draw";
})(MatchResult || (MatchResult = {}));
export function toAnchorMarketType(marketType) {
    switch (marketType) {
...
export function toAnchorMarketStatus(status) {
    switch (status) {
...
export function toAnchorMatchResult(result) {
    switch (result) {
...
export function validateResolveMarketInput(input) {
    if (!input.matchId || input.matchId.trim() === "") {
        throw new Error("matchId is required");
    }
    if (input.marketId < 0 || input.marketId > 255) {
...
export function validateUpdateMarketStatusInput(input) {
    if (!input.matchId || input.matchId.trim() === "") {
        throw new Error("matchId is required");
    }
    if (input.marketId < 0 || input.marketId > 255) {

...
async function main() {
    ...
    try {
        ...
        const ata = await getOrCreateAssociatedTokenAccount(connection, payer, mintPubkey, destPubkey);
        console.log(`🎉 ATA Ready: ${ata.address.toBase58()}`);
        const mintAmount = amount * 1_000_000; // 6 Decimals
        const tx = await mintTo(connection, payer, mintPubkey, ata.address, payer, mintAmount);
        console.log(`\n=========================================`);

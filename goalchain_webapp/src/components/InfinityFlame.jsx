 ...
 */
function generateFlamePath(centerX, bottomY, height, width, turbulence, seed) {
...
function createEmberParticles(count, centerX, bottomY, flameWidth, flameHeight) {
...
export const InfinityFlame = ({ intensity = 0.5, width = 400, height = 280, enableParticles = true, className = '', }) => {
    ...
    useEffect(() => {
        ...
        particlesRef.current = createEmberParticles(particleCount, centerX, bottomY, flameWidth, flameHeight);
    }, [width, height]);
    // Animation loop (canvas fallback)
    useEffect(() => {
        if (prefersReducedMotion || smilSupported)
            return;
        if (!enableParticles)
            return;
        ...
        const animate = (now) => {
            ...
            const glowGradient = ctx.createRadialGradient(centerX, bottomY, 0, centerX, bottomY, flameWidth * 0.8);
  ...
  `;
    ...
    return (<div className={containerClasses} style={{ width, height, minHeight: height }}>
      {/* Canvas fallback for reduced motion or no SMIL support */}
      {(prefersReducedMotion || !smilSupported) && enableParticles && (<canvas ref={canvasRef} className="infinity-flame-canvas flame-layer" width={width} height={height} aria-hidden="true"/>)}

      {/* Primary SVG Animation (SMIL) */}
      <svg ref={svgRef} className="infinity-flame-svg flame-layer" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" aria-hidden="true" role="img" aria-label={`Infinity Burn flame visualization, intensity ${Math.round(intensity * 100)}%`}>
        <defs>
          {getFlameGradient()}
          {getEmberGradient()}
        ...
        {/* Base ember glow */}
        <ellipse cx={centerX} cy={bottomY - flameHeight * 0.05} rx={flameWidth * 0.7} ry={flameHeight * 0.12} fill="url(#emberGrad)" className="flame-layer">
          {!prefersReducedMotion && smilSupported && (<g>
              <animate attributeName="opacity" values={`0.2;${0.4 + intensity * 0.3};0.2`} dur={`${3 / (0.5 + intensity * 0.5)}s`} repeatCount="indefinite"/>
              <animate attributeName="rx" values={`${flameWidth * 0.65};${flameWidth * 0.75};${flameWidth * 0.65}`} dur={`${4 / (0.5 + intensity * 0.5)}s`} repeatCount="indefinite"/>
            </g>)}
        </ellipse>

        {/* Main flame shape with SMIL morphing */}
        <path fill="url(#flameGrad)" filter="url(#flameGlow)" className="flame-layer" d={flamePaths[1]} // Default to middle path
    >
          {!prefersReducedMotion && smilSupported && (<g>
              <animate attributeName="d" values={flamePaths.join(';')} dur={`${3.5 / (0.5 + intensity * 0.5)}s`} repeatCount="indefinite" keyTimes="0;0.5;1" calcMode="spline" keySplines="0.4 0 0.2 1;0.4 0 0.2 1"/>
              <animate attributeName="opacity" values={`0.85;1;${0.8 + intensity * 0.1}`} dur="0.12s" repeatCount="indefinite"/>
            </g>)}
        </path>
        ...
        {[1, 2, 3, 4, 5, 6].map((_, i) => {
            ...
            return (<circle key={i} cx={startX} cy={startY} r={particleSize} fill="url(#particleGrad)" className="flame-layer" opacity={0.7 * intensity + 0.2}>
              {!prefersReducedMotion && smilSupported && (<g>
                  <animateTransform attributeName="transform" type="translate" values={`0 0; ${driftX} ${driftY}; 0 0`} dur={`${2 + i * 0.25 + intensity * 0.5}s`} repeatCount="indefinite" calcMode="spline" keySplines="0.25 0.1 0.25 1"/>
                  <animate attributeName="opacity" values={`0.7;0;0.7`} dur={`${2 + i * 0.25 + intensity * 0.5}s`} repeatCount="indefinite"/>
                  <animate attributeName="r" values={`${particleSize};${particleSize * 0.3};${particleSize}`} dur={`${2 + i * 0.25 + intensity * 0.5}s`} repeatCount="indefinite"/>
                </g>)}
            </circle>);
        })}
      </svg>

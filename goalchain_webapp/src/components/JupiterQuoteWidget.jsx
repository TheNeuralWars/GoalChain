...
async function retryWithBackoff(fn, options = {
    ...
    for (let attempt = 0; attempt <= options.maxRetries; attempt++) {
        try {
            ...
            const delay = Math.min(options.baseDelayMs * Math.pow(2, attempt) + Math.random() * 1000, options.maxDelayMs);
...
export function JupiterQuoteWidget() {
    ...
    const fetchQuote = useCallback(async () => {
        ...
        setQuote(null);
        try {
            const res = await retryWithBackoff(() => fetchWithTimeout(`${API_BASE}/api/solana/jupiter/quote`, {
                method: 'POST',
    ...
    return (<div className="p-6 max-w-xl border rounded-xl bg-white shadow-sm">
      ...
      <div className="flex flex-col gap-4 mb-6">
        <div>
          ...
          <input type="text" value={inputMint} onChange={(e) => setInputMint(e.target.value)} className="w-full border px-3 py-2 rounded-md text-sm"/>
        </div>

        <div>
          <label className="block text-sm font-medium mb-1">Output Mint</label>
          <input type="text" value={outputMint} onChange={(e) => setOutputMint(e.target.value)} className="w-full border px-3 py-2 rounded-md text-sm"/>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1">Amount</label>
            <input type="text" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full border px-3 py-2 rounded-md text-sm"/>
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Slippage (bps)</label>
            <input type="text" value={slippageBps} onChange={(e) => setSlippageBps(e.target.value)} className="w-full border px-3 py-2 rounded-md text-sm"/>
          </div>
        </div>

        <button onClick={fetchQuote} disabled={loading} className="mt-2 bg-black text-white py-2.5 rounded-lg hover:bg-neutral-800 disabled:opacity-50">
          {loading ? 'Fetching quote...' : 'Get Quote'}
        </button>
      </div>

      {error && (<div className="text-red-600 text-sm mb-4">
          Error: {error}
        </div>)}

      {quote && (<div className="bg-neutral-50 p-4 rounded-lg text-sm">

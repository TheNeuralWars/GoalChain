...
export const HealthCheckCard = ({ checks, configDriftReasons, className = '', }) => {
    return (<div className={`glass-card-premium ${className}`}>
      <h3 style={{
            margin: '0 0 1rem',
            fontSize: '1.1rem',
            color: '#fff',
      ...
      <div className="health-checks-grid" role="list" aria-label="Health check results">
        {checks.map((check, idx) => (<div key={idx} className={`health-check-card ${check.pass ? 'pass' : 'fail'}`} role="listitem" style={{ ['--dot-color']: check.pass ? 'var(--primary-neon)' : 'var(--accent-red)' }}>
            <div className="health-check-main">
              <span className="health-check-dot" aria-hidden="true" style={{ ['--dot-color']: check.pass ? 'var(--primary-neon)' : 'var(--accent-red)' }}/>
      ...
      {configDriftReasons.length > 0 && (<div className="config-drift-alert" role="alert">
          <div className="config-drift-title">Config Drift Detected</div>
          <ul className="config-drift-list">
            {configDriftReasons.map((reason, i) => (<li key={i} style={{ marginBottom: '0.25rem' }}>{reason}</li>))}
          </ul>
        </div>)}
    </div>);
};

...
export const YieldCalculator = ({ selectedStrategy, strategies, vaultBuybackCoverage = 0.25, gchPrice = 0.001, solPrice = 150, onStakeAmountChange, onStakeTokenChange, className = '', }) => {
    const [stakeInput, setStakeInput] = useState('1000');
    const [stakeToken, setStakeToken] = useState('GCH');
    ...
    const handleStakeInputChange = useCallback((e) => {
    ...
    return (<div className={`yield-calculator-premium glass-card-premium ${className}`}>
      <h2 style={{
            margin: '0 0 1.5rem',
            fontSize: '1.25rem',
            color: '#fff',
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          {strategies.map(s => (<button key={s.id} onClick={() => { }} disabled style={{
                flex: 1,
                minWidth: '160px',
      ...
      <div className="yield-input-group">
        <div>
          ...
          <div className="yield-input-wrapper">
            <input type="number" value={stakeInput} onChange={handleStakeInputChange} className="yield-input-premium" style={{ flex: 1, fontSize: '1.1rem', fontWeight: 700, fontFamily: 'JetBrains Mono, monospace' }} min="0" step="10" placeholder="0" aria-label={`Stake amount in ${stakeToken}`}/>
            <div className="token-switcher" role="group" aria-label="Select stake token">
              <button type="button" className={`token-btn gch ${stakeToken !== 'GCH' ? 'inactive' : ''}`} onClick={() => handleTokenChange('GCH')} aria-pressed={stakeToken === 'GCH'}>
                $GCH
              </button>
              <button type="button" className={`token-btn sol ${stakeToken !== 'SOL' ? 'inactive' : ''}`} onClick={() => handleTokenChange('SOL')} aria-pressed={stakeToken === 'SOL'}>
                SOL
              </button>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="preset-chips" role="group" aria-label="Preset stake amounts">
            {[100, 500, 1000, 5000, 10000, 50000].map(amt => (<button key={amt} type="button" className="preset-chip" onClick={() => handlePresetClick(amt)} aria-label={`Stake ${amt >= 1000 ? amt / 1000 + 'K' : amt} ${stakeToken}`}>

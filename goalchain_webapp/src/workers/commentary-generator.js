...
const EXTENDED_TEMPLATES = [
    {
        ...
        templates: [
            ...
            "🏟️ Bienvenidos al GoalChain World Cup 2026. {homeTeam} recibe a {awayTeam}.",
        ],
...
export class CommentaryGenerator {
    ...
    wasmInstance = null;
    rngSeed = Date.now() + 1;
    templateIndices = new Map();
    constructor() {
        this.prepareTemplateIndices();
    }
    prepareTemplateIndices() {
    ...
    selectWeightedTemplate(data, weights, preferredIndex) {
    ...
    renderCommentary(template, event, context, weights) {
        ...
        if (weights.celebration === maxWeight && weights.celebration > 0.5)
            emotion = 'celebration';
        else if (weights.excitement === maxWeight && weights.excitement > 0.6)
            emotion = 'excited';
        else if (weights.tension === maxWeight && weights.tension > 0.6)
            emotion = 'tense';
        else if (weights.analytical === maxWeight && weights.analytical > 0.5)
            emotion = 'analytical';
        else if (event.type === 'GOAL')
            emotion = 'celebration';
        else if (event.type === 'SHOT' || event.type === 'PENALTY')
            emotion = 'tense';
        return {
    ...
    callWasmParser(event, context) {
        if (!this.wasmInstance)
            throw new Error('WASM not initialized');
        const memory = this.wasmInstance.exports.memory;
        const parseFn = this.wasmInstance.exports.parse_commentary;
        // Allocate output buffer in WASM memory
        const outputPtr = this.wasmInstance.exports.alloc(64);
        ...
        parseFn(this.eventTypeToIndex(event.type), homeStrength, awayStrength, playerSpeed, playerShotPower, playerStamina, context.minute, scoreDiff, isHome, outputPtr);
        ...
        this.wasmInstance.exports.free(outputPtr);
        return result;
    }
    eventTypeToIndex(type) {
        const types = [

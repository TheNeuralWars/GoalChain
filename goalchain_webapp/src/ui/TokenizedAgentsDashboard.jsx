...
function StatusDot({ status }) {
    const colors = { active: '#14f195', thinking: '#fbbf24', idle: '#64748b' };
    const labels = { active: 'ACTIVE', thinking: 'THINKING', idle: 'IDLE' };
    return (<span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
      <span style={{
...
function AgentCard({ agent, lang }) {
    ...
    return (<div className="glass-card" style={{
        ...
        }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = `0 8px 24px ${agent.color}15`; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none'; }}>
      {/* Header */}
...
export function TokenizedAgentsDashboard() {
    ...
    return (<div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
      {/* Header */}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
        {AGENTS.map(agent => (<AgentCard key={agent.id} agent={agent} lang={language}/>))}
      </div>
    </div>);
}

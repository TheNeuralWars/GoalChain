...
import { claimFixturePayout, fetchFixtures, fetchUserBets, placeFixtureBet, refundFixtureBet, } from '../lib/goalchainClient';
import { useTranslation } from '../i18n';
import { explorerTxUrl } from '../lib/explorerLinks';
import wcFixtureData from '../config/wc2026_fixture.json';
const LOCAL_FIXTURES = wcFixtureData;
const ROUND_ORDER = [
    'round_of_32',
    'round_of_16',
    'quarterfinal',
...
export const FixturesPanel = () => {
    ...
    const betForFixture = (fixturePubkey) => userBets.find((b) => b.fixture === fixturePubkey);
    const openExplorerLink = (signature) => {
        const url = explorerTxUrl(signature, connection);
        window.open(url, '_blank', 'noopener,noreferrer');
        return url;
    };
    const handleBet = async (fixturePubkey, side) => {
        if (!wallet.publicKey) {
    ...
    const handleClaim = async (fixturePubkey) => {
        if (!wallet.publicKey) {
            addToast('warn', '¡Conecta tu wallet primero!');
            return;
        }
    ...
    const handleRefund = async (fixturePubkey) => {
        if (!wallet.publicKey) {
            addToast('warn', '¡Conecta tu wallet primero!');
            return;
        }
    ...
    return (<div className="gc-fixtures-root">
      {/* Toast container */}
      <div className="gc-toast-stack" aria-live="polite">
        {toasts.map((tk) => (<div key={tk.id} className={`gc-toast gc-toast--${tk.type}`}>
            {tk.message}
          </div>))}
      </div>
      ...
      <div className="gc-fixtures-toolbar">
        <div className="gc-fixtures-viewswitch" role="tablist" aria-label="Fixture view">
          <button role="tab" aria-selected={view === 'list'} className={`gc-viewswitch-btn ${view === 'list' ? 'gc-viewswitch-btn--active' : ''}`} onClick={() => setView('list')}>
            <span aria-hidden>📋</span>
            {t('fix_view_list')}
          </button>
          <button role="tab" aria-selected={view === 'bracket'} className={`gc-viewswitch-btn ${view === 'bracket' ? 'gc-viewswitch-btn--active' : ''}`} onClick={() => setView('bracket')}>
            <span aria-hidden>🌳</span>
            {t('fix_view_bracket')}
          </button>
        </div>
      ...
      {view === 'bracket' ? (<KnockoutBracket />) : (<>
          {loading ? (<div className="gc-fixtures-loading">{t('fix_loading')}</div>) : (<div className="fixtures-container gc-fixtures-list">
              ...
              {fixtures.map((f) => {
                    ...
                    return (<div key={f.pubkey} className={`gc-fixture-card glass-card gc-fixture-card--${f.status}`}>
                    ...
                    {(f.scoreA !== undefined || f.scoreB !== undefined) && (<div className="gc-fixture-score">
                        <span className="gc-score-num">{f.scoreA ?? 0}</span>
                        <span className="gc-score-sep">:</span>
                        <span className="gc-score-num">{f.scoreB ?? 0}</span>
                        {f.status === 'live' && f.minute !== undefined && (<span className="gc-live-minute">
                            <span className="gc-live-dot" aria-hidden/>
                            Min {f.minute}'
                          </span>)}
                      </div>)}

                    {mine && (<div className="gc-fixture-mine">
                        Tu apuesta: {mine.amountBaseUnits} base units · predicción {mine.prediction}
                        {mine.claimed ? ' · cobrada' : ' · pendiente'}
                      </div>)}

                    {canBet && (<div className="gc-fixture-betinput">
                        <input type="text" placeholder="Monto (ej: 1.5)" value={betAmounts[f.pubkey] ?? ''} onChange={(e) => setBetAmounts((prev) => ({ ...prev, [f.pubkey]: e.target.value }))}/>
                      </div>)}

                    <div className="bet-actions">
                      {canBet && (<>
                          <button className="gc-btn gc-btn--green" disabled={submittingFor === `${f.pubkey}:A`} onClick={() => handleBet(f.pubkey, 'A')}>
                            {submittingFor === `${f.pubkey}:A` ? 'Enviando...' : `Gana ${f.teamA}`}
                          </button>
                          <button className="gc-btn gc-btn--ghost" disabled={submittingFor === `${f.pubkey}:Draw`} onClick={() => handleBet(f.pubkey, 'Draw')}>
                            {submittingFor === `${f.pubkey}:Draw` ? 'Enviando...' : 'Empate'}
                          </button>
                          <button className="gc-btn gc-btn--green" disabled={submittingFor === `${f.pubkey}:B`} onClick={() => handleBet(f.pubkey, 'B')}>
                            {submittingFor === `${f.pubkey}:B` ? 'Enviando...' : `Gana ${f.teamB}`}
                          </button>
                        </>)}
                      {canClaim && (<button className="gc-btn gc-btn--purple" disabled={submittingFor === `${f.pubkey}:claim`} onClick={() => handleClaim(f.pubkey)}>
                          {submittingFor === `${f.pubkey}:claim` ? 'Cobrando...' : 'Cobrar ganancia'}
                        </button>)}
                      {canRefund && (<button className="gc-btn gc-btn--amber" disabled={submittingFor === `${f.pubkey}:refund`} onClick={() => handleRefund(f.pubkey)}>
...
const KnockoutBracket = () => {
    ...
    return (<div className="gc-bracket-scroll">
      <div className="gc-bracket" role="tree" aria-label="World Cup 2026 knockout bracket">
        {rounds.map((round) => (<section key={round.key} className="gc-bracket-column" role="group">
            <h3 className="gc-bracket-column-title">{round.label}</h3>
            <div className="gc-bracket-matches">
              {round.matches.length === 0 ? (<div className="gc-bracket-empty">—</div>) : (round.matches.map((m) => (<BracketCard key={m.id} match={m}/>)))}
            </div>
          </section>))}
      </div>
    </div>);
...
const BracketCard = ({ match }) => {
    ...
    return (<article className={`gc-bcard glass-card gc-bcard--${status} ${isLive ? 'gc-bcard--live' : ''}`}>
      ...
      <div className="gc-bcard-foot">
        {isLive ? (<span className="gc-bcard-minute">
            <span className="gc-live-dot" aria-hidden/> {t('fix_live')}
          </span>) : isDone ? (<span className="gc-bcard-foot-label">{t('fix_ft')}</span>) : (<span className="gc-bcard-foot-label gc-bcard-foot-label--muted">

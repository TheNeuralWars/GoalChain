...
export const CreateUser: React.FC<CreateUserProps> = ({ onUserCreated }) => {
  ...
  const validateUsername = (v: string) => {
    if (v.length < 3) return t('create_user_username_min_length');
    if (v.length > 20) return t('create_user_username_max_length');
    if (!/^[a-zA-Z0-9_]+$/.test(v)) return t('create_user_username_invalid_chars');
    return '';
  };
  ...
  const handleSubmit = async () => {
    ...
    const userData = {
      ...
      joinedDate: t('create_user_joined_date'),
      bio: t('create_user_bio', { role: selectedRoleObj.label }),
      location: t('create_user_location'),
      following: [],
      accentColor: '#14f195',
    };
    setUser(userData);
      ...
      <div className="create-user-card">
        ...
        {step === 2 && (
          <div className="step-content" id="step-identity">
            ...
            <div className="step-nav">
              <button className="btn-secondary" onClick={() => setStep(1)}>← {t('create_user_back')}</button>
              <button
                className="btn-primary"
                id="step2-next"
        ...
        {step === 3 && (
          <div className="step-content" id="step-role">
            ...
            <div className="step-nav">
              <button className="btn-secondary" onClick={() => setStep(2)}>← {t('create_user_back')}</button>
              <button

...
export function Toaster() {
    ...
    const push = useCallback((icon, text, variant = 'success') => {
        const id = ++toastSeq;
    ...
    useGameEvent('penalty:goal', ({ scored }) => {
        push('⚽', `¡${t('toast_goal') || 'GOAL'}! +${scored}`, 'success');
    });
    useGameEvent('penalty:miss', () => {
        push('🧤', t('toast_save') || 'Saved!', 'info');
    });
    useGameEvent('pack:opened', ({ rarity }) => {
        const icon = RARITY_ICONS[rarity] ?? '🎁';
        const label = rarity !== 'unknown' ? rarity.toUpperCase() : '';
        push(icon, `${t('toast_pack') || 'New card'}${label ? ': ' + label : ''}`, 'premium');
    });
    return (<div className="gc-toaster" role="status" aria-live="polite">
      {toasts.map((toast) => (<div key={toast.id} className={`gc-toast gc-toast--${toast.variant} glass-strong`}>

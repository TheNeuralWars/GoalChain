...
function fmtDate(ts, lang = 'en') { return new Date(ts).toLocaleString(lang === 'es' ? 'es-ES' : 'en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }); }
// ─────────────────────────────────────────────────────────────────────────────
// Sub-components
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
function SwarmTopology({ nodes }) {
    const { language } = useTranslation();
    const ceo = nodes[0];
    const others = nodes.slice(1);
    return (<div style={{ position: 'relative', minHeight: '180px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '12px 0' }}>
      {/* CEO Node */}
      <div style={{
      ...
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', justifyContent: 'center' }}>
        {others.map(n => (<div key={n.name} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
            {/* Connector line */}
            <div style={{
...
// ─────────────────────────────────────────────────────────────────────────────
function EconomyDiagram({ highlighted }) {
    const { language } = useTranslation();
    ...
    return (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', flexWrap: 'wrap' }}>
      {items.map((item, idx) => (<React.Fragment key={item.label}>
          <div style={{
          ...
          {idx < items.length - 1 && (<div style={{ color: flowColor, fontSize: '1rem', transition: 'color 0.5s ease' }}>→</div>)}
        </React.Fragment>))}
    </div>);
}
// ─────────────────────────────────────────────────────────────────────────────
...
export function CorporateAutopilot() {
    ...
    const { language } = useTranslation();
    const tText = useCallback((en, es) => (language === 'es' ? es : en), [language]);
    // Core state
    ...
    const [swarmNodes, setSwarmNodes] = useState(INITIAL_SWARM_NODES);
    const [swarmHops, setSwarmHops] = useState(0);
    const getInitialLogs = useCallback((lang) => lang === 'es' ? [
    ...
    const fetchThreadState = useCallback(async (id) => {
        try {
    ...
    const handleSendThread = async (e, isReply) => {
        e.preventDefault();
    ...
    const setNodeStatus = useCallback((name, status) => {
    ...
    const runScenario = useCallback((scenarioId) => {
        ...
        if (scenarioId === 'nft_sale_cycle') {
            ...
            t(2000, () => addLog(tText('[GoalChain Economy] 90% ($8.99) to manager pool. 10% ($1.00) routing to Agent Wallet...', '[Economía GoalChain] 90% ($8.99) al pool de mánagers. 10% ($1.00) enrutado al monedero de agentes...')));
        ...
        if (scenarioId === 'player_quorum') {
            setSwarmNodes(prev => prev.map(n => ({ ...n, status: 'idle' })));
            setSwarmHops(0);
    ...
    const handleCheckout = async (type) => {
        ...
        try {
            ...
            if (r.ok) {
                ...
                window.open('https://checkout.stripe.com/c/pay/cs_test_mock_12345', '_blank');
        }
        catch {
            window.open('https://checkout.stripe.com/c/pay/cs_test_mock_12345', '_blank');
        }
    ...
    // Render
    // ─────────────────────────────────────────────
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%', color: '#fff', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '0.75rem', color: '#fbbf24', fontWeight: 700, marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
          {tText('⚙️ Economic Flywheel — NFT Revenue → 10% → Agent Wallet → Self-Sustaining Operations', '⚙️ Volante Económico — Ingresos de NFT → 10% → Monedero de Agente → Operaciones Auto-Sostenibles')}
        </div>
      ...
      <section style={{
        ...
        <h3 style={{ margin: '0 0 14px 0', fontSize: '0.9rem', fontWeight: 800, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
          {tText('🎮 Live Simulation Console — Trigger a Real Business Event', '🎮 Consola de Simulación en Vivo — Activar un Evento Empresarial Real')}
        </h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
          {Object.entries(SCENARIOS).map(([id, s]) => {
            const isActive = activeScenario === id;
            const isDisabled = !!activeScenario && !isActive;
            return (<button key={id} onClick={() => runScenario(id)} disabled={!!activeScenario} style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '1.5rem' }}>
        ...
        <section style={{
          ...
          <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 800, color: '#14f195', borderBottom: '1px solid rgba(20,241,149,0.12)', paddingBottom: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            {activeScenario && (<span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.65rem', color: '#ef4444', fontWeight: 800 }}>
          ...
          <div style={{
            ...
            {terminalLogs.map((log, idx) => (<div key={idx} style={{
                whiteSpace: 'pre-wrap',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '1.5rem' }}>
        ...
        <section style={{
          ...
          <div style={{ display: 'flex', gap: '8px', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '12px' }}>
            {[['wallet', tText('💳 Agent Wallet', '💳 Monedero Agente')], ['ledger', tText('📊 Corp Ledger', '📊 Libro Corp')], ['safety', tText('🛡️ NemoClaw', '🛡️ NemoClaw')], ['threads', tText('📟 Swarm Threads', '📟 Hilos Swarm')]].map(([tab, label]) => (<button key={tab} onClick={() => setActiveTab(tab)} style={{
          ...
          {/* AGENT WALLET TAB */}
          {activeTab === 'wallet' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {/* Wallet balance bar */}
              <div>
              ...
              <div>
                ...
                return (<div key={ev.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(20,241,149,0.04)', border: '1px solid rgba(20,241,149,0.08)', borderRadius: '8px', padding: '6px 10px', fontSize: '0.7rem' }}>
                        <div>
              ...
              <div>
                ...
                return (<div key={ev.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(239,68,68,0.04)', border: '1px solid rgba(239,68,68,0.08)', borderRadius: '8px', padding: '6px 10px', fontSize: '0.7rem' }}>
                        <div>
          ...
          {/* CORP LEDGER TAB */}
          {activeTab === 'ledger' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '220px', overflowY: 'auto' }}>
                ...
                const translateDescription = (desc) => {
                    ...
                    const translations = {
                        ...
                        'Domain Purchase: play-goalchain.com': 'Compra de Dominio: play-goalchain.com',
                ...
                return (<div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '8px 12px', borderRadius: '8px', fontSize: '0.73rem' }}>
                      <div>
          ...
          {/* NEMOCLAW TAB */}
          {activeTab === 'safety' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          ...
          {/* SWARM THREADS TAB */}
          {activeTab === 'threads' && (<div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {selectedThreadId === null ? (<>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '240px', overflowY: 'auto' }}>
                    {loadingThreads && (<div style={{ fontSize: '0.7rem', color: '#64748b', textAlign: 'center', padding: '10px' }}>
                        {tText('Loading threads...', 'Cargando hilos...')}
                      </div>)}
                    {!loadingThreads && threads.length === 0 && (<div style={{ fontSize: '0.7rem', color: '#64748b', textAlign: 'center', padding: '10px', background: 'rgba(255,255,255,0.01)', border: '1px dashed rgba(255,255,255,0.08)', borderRadius: '8px' }}>
                        {tText('No active threads. Launch a new swarm node below.', 'Sin hilos activos. Inicia un nuevo nodo de enjambre abajo.')}
                      </div>)}
                    {threads.map(t => (<div key={t.thread_id} onClick={() => setSelectedThreadId(t.thread_id)} style={{
                        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                        background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)',
                        borderRadius: '8px', padding: '8px 10px', fontSize: '0.7rem', cursor: 'pointer',
                        transition: 'all 0.2s',
                    }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = '#9945ff'; e.currentTarget.style.background = 'rgba(153,69,255,0.02)'; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'; e.currentTarget.style.background = 'rgba(255,255,255,0.02)'; }}>
                  ...
                  <form onSubmit={(e) => handleSendThread(e, false)} style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '8px', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '12px' }}>
                    ...
                    <textarea value={newThreadObjective} onChange={(e) => setNewThreadObjective(e.target.value)} placeholder={tText("Enter swarm objective (e.g. 'audita solana anchor program')...", "Introduce el objetivo del enjambre (ej: 'audita programa anchor solana')...")} style={{
                ...
                }} onFocus={(e) => e.target.style.borderColor = '#9945ff'} onBlur={(e) => e.target.style.borderColor = 'rgba(255,255,255,0.08)'}/>
                    <button type="submit" disabled={sendingThread || !newThreadObjective.trim()} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '8px' }}>
                    <button onClick={() => { setSelectedThreadId(null); setSelectedThreadState(null); }} style={{ background: 'transparent', border: 'none', color: '#00e0ff', fontSize: '0.7rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', padding: 0 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '200px', overflowY: 'auto', paddingRight: '4px' }}>
                    {selectedThreadState && selectedThreadState.messages && selectedThreadState.messages.map((m, idx) => {
                    ...
                    {!selectedThreadState && (<div style={{ fontSize: '0.7rem', color: '#64748b', textAlign: 'center', padding: '10px' }}>
                  ...
                  <form onSubmit={(e) => handleSendThread(e, true)} style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '8px', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '12px' }}>
                    <textarea value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder={tText('Type your response to this thread...', 'Escribe tu respuesta a este hilo...')} style={{
                    width: '100%', height: '54px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)',
                    borderRadius: '8px', padding: '8px', color: '#fff', fontSize: '0.72rem', fontFamily: 'inherit', resize: 'none',
                    outline: 'none', transition: 'border-color 0.2s',
                }} onFocus={(e) => e.target.style.borderColor = '#9945ff'} onBlur={(e) => e.target.style.borderColor = 'rgba(255,255,255,0.08)'}/>
                    <button type="submit" disabled={sendingThread || !replyText.trim()} style={{
                    padding: '8px 16px', borderRadius: '8px', fontSize: '0.72rem', fontWeight: 700,
        ...
        ].map(item => (<div key={item.labelEn} style={{
      ...
      {showQuorumPanel && (<section style={{
          ...
          <PlayerQuorumPanel initialOperation={quorumOperation} onQuorumReached={(result) => {
                const signers = result.squad_players.map((p) => p.name).join(', ');
                addLog(tText(`[Quorum] ✅ AUTHORIZED by ${result.squad_players.length} signers: ${signers} (${result.squad_gp.toFixed(1)} GP)`, `[Cuórum] ✅ AUTORIZADO por ${result.squad_players.length} firmantes: ${signers} (${result.squad_gp.toFixed(1)} GP)`));
                setTimeout(() => {
                    addLog(tText('[CEO] Quorum confirmed. Stripe payment $200 authorized by squad.', '[CEO] Cuórum confirmado. Pago de Stripe de $200 autorizado por la plantilla.'));
                    setNodeStatus('CEO', 'thinking');
                    setSwarmHops(2);
                }, 500);
                setTimeout(() => addLog(tText('[Stripe] Processing: $200 payment for Render.com hosting...', '[Stripe] Procesando: pago de $200 para alojamiento Render.com...')), 1800);
                setTimeout(() => {
                    addLog(tText('[Stripe] ✅ SUCCESS: $200 paid. Render.com hosting renewed 30 days.', '[Stripe] ✅ ÉXITO: $200 pagados. Alojamiento Render.com renovado por 30 días.'));
                    setAgentWallet((p) => ({
                        ...p,
                ...
                setShowQuorumPanel(false);
                addLog(tText('[System] Quorum panel dismissed. Operation cancelled by manager.', '[Sistema] Panel de cuórum descartado. Operación cancelada por el mánager.'));
            }}/>
        </section>)}

...
import { LayeredNftCard } from './LayeredNftCard';
function mapPlayerToDetailed(p) {
    let role = 'Delantero';
    if (p.position === 'MID')
        role = 'Centrocampista';
...
export const SquadGallery = () => {
    const [selectedPlayer, setSelectedPlayer] = useState(null);
    const [activeSystem, setActiveSystem] = useState('4-3-3');
    ...
    const loadInventory = async () => {
        try {
            ...
            if (inventory.length > 0) {
                const detailed = inventory.map((p) => mapPlayerToDetailed(p));
                setSquad(detailed);
                setIsDemo(false);
            }
            else {
    ...
    const getSystemCoefficient = (player, system) => {
        return player.tacticalWeights[system];
    };
    const getAdjustedOvr = (player, system) => {
        return Math.round(player.score * getSystemCoefficient(player, system));
    };
    return (<div className="squad-gallery" style={{ marginTop: '0.5rem', textAlign: 'left' }}>
      {isDemo && (<div style={{
      ...
      <div style={{
        ...
        {squad.map((p, i) => (<LayeredNftCard key={i} player={p.rawPlayer} onAnalyze={() => {
                setSelectedPlayer(p);
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }}>
                ...
                boxShadow: 'inset 0 0 20px rgba(255, 255, 255, 0.02)'
                  ...
                  <svg width="120" height="130" viewBox="0 0 100 110" style={{ overflow: 'visible' }}>
                    ...
                    <path d="M 50 5 L 85 20 L 85 70 L 50 105 L 15 70 L 15 20 Z" fill="url(#modalShieldGrad)" stroke="var(--primary-neon)" strokeWidth="2.5" style={{ filter: 'drop-shadow(0 4px 15px rgba(0,255,127,0.25))' }}/>
                ...
                return (<button key={system} onClick={() => setActiveSystem(system)} style={{

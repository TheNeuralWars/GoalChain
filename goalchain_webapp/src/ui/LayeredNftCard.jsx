...
export const LayeredNftCard = ({ player, isFav = false, onToggleFav, isMarketplace = false, onBuyCash, onBuySol, onAnalyze, isSolOffline = false, }) => {
    ...
    // Resolve backgrounds
    const stadiumBgImg = `https://goalchain.fun/assets/img/stadiums/${BG_IMAGE_MAP[player.bg_type || ''] || 'bg_common_street.png'}`;
    const stadiumBgVideo = `https://goalchain.fun/assets/video/stadiums/${BG_VIDEO_MAP[player.bg_type || ''] || 'dome_kronos_vertical.mp4'}`;
    return (<div className={`nft-card-3d ${isFlipped ? 'is-flipped' : ''}`} data-rarity={player.rarity.toLowerCase()} onMouseMove={handleMouseMove} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => {
            setIsHovered(false);
            setCoords({ x: 50, y: 50 });
      ...
      {onToggleFav && (<div className="favorite-heart is-fav" onClick={(e) => {
                e.stopPropagation();
      ...
      <div className="card-inner">
        ...
        <div className="card-front">
          <div className="layer layer-bg" style={{ position: 'relative', overflow: 'hidden', width: '100%', height: '100%' }}>
            <img src={stadiumBgImg} alt="Stadium Background" className="bg-img" style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
            <video ref={videoRefFront} className="bg-video-hover" src={stadiumBgVideo} muted playsInline preload="none" style={{
            position: 'absolute',
            inset: 0,
            width: '100%',
            height: '100%',
          ...
          <div className="layer layer-base">
            <img src={imgPath} alt={player.name} loading="lazy" onError={(e) => {
            e.target.style.display = 'none';
            const baseEl = e.target.parentElement;
            if (baseEl)
                baseEl.classList.add('no-image');
        }}/>
        ...
        <div className="card-back" style={{ padding: 0, position: 'relative' }}>
          ...
          <div className="layer layer-bg" style={{ position: 'absolute', inset: 0, overflow: 'hidden', borderRadius: '18px' }}>
            <img src={stadiumBgImg} alt="Stadium Background Back" className="bg-img" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.35) contrast(1.1)' }}/>
            <video ref={videoRefBack} className="bg-video-hover" src={stadiumBgVideo} muted playsInline preload="none" style={{
            position: 'absolute',
            inset: 0,
            width: '100%',
            height: '100%',
          ...
          <div className="back-content" style={{
            ...
            marginBottom: '10px'
              ...
              <div style={{ marginBottom: '8px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                  {player.meta?.visual_effect && (<span style={{
                ...
                fontWeight: 700
                  ...
                  {player.traits && player.traits.length > 0 ? (player.traits.map((t, idx) => (<span key={idx} style={{
                ...
                fontWeight: 700
                        ...
                        ⚡ {t}
                      </span>))) : (<span style={{ color: '#64748b', fontSize: '0.58rem', fontStyle: 'italic' }}>Sin rasgos especiales</span>)}
                </div>
              </div>
            </div>
            ...
            marginBottom: '8px'
              ...
              {/* Action Buttons */}
              {isMarketplace ? (<div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                if (onBuyCash)
                  ...
                  <button className="btn-buy btn-buy-sol" onClick={(e) => {
                e.stopPropagation();
                if (onBuySol)
                    onBuySol();
            }} style={{
            ...
            }} disabled={isSolOffline} title={isSolOffline ? 'Tesorería no disponible' : undefined}>
                    ⚡ COMPRAR CON SOL: {player.price || '0.5 SOL'}
                  </button>
                </div>) : (<div style={{ display: 'flex', gap: '8px' }}>
                  {onAnalyze && (<button className="btn-buy btn-analyze" onClick={(e) => {
                    e.stopPropagation();
                    onAnalyze();
                }} style={{
                    flex: 1,

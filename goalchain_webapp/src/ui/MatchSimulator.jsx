...
import { useMatchSim, createDemoTeams } from '../hooks/useMatchSim';
...
export function MatchSimulator() {
    ...
    const { state, events, commentary, isRunning, isInitialized, speed, init, start, pause, resume, stop, setSpeed, setTactic, energyBoost, substitute } = useMatchSim();
    const [showControls, setShowControls] = useState(true);
    const commentaryEndRef = useRef(null);
    const [homeTeam, setHomeTeam] = useState(null);
    const [awayTeam, setAwayTeam] = useState(null);
    // States for stakes and tactics
    const [gchBalance, setGchBalance] = useState(1000);
    const [isFallbackMock, setIsFallbackMock] = useState(true);
    const [activeTactic, setActiveTacticState] = useState('normal');
    const [reserves, setReserves] = useState([]);
    const [selectedSubOutId, setSelectedSubOutId] = useState(null);
    ...
    const updateGchBalance = (newBalance) => {
    ...
    const syncSquadAndBalance = () => {
        ...
        const tName = user ? `${user.avatar} ${user.username} FC` : 'GoalChain FC';
        ...
        if (state && state.minute >= 90 && !matchFinished) {
            ...
            if (fulltimeEvent) {
                ...
                if (state.homeScore > state.awayScore) {
                    ...
                    if (user) {
                        const currentXp = user.xp || 4210;
                        setUser({ ...user, xp: currentXp + 25 });
                    }
                }
                else {
                    // Draw/Loss: +5 EXP
    ...
    const handleTacticChange = (tactic) => {
        if (!isRunning)
            return;
        setActiveTacticState(tactic);
        setTactic(tactic);
    };
    const handleEnergyBoost = (playerId) => {
        if (!isRunning)
            return;
        const current = getGchBalance();
        if (current < 15) {
    ...
    const handleSubSelect = (playerIn) => {
        if (!selectedSubOutId)
            return;
    ...
    if (!isInitialized) {
        return (<div className="glass-card" style={{ padding: '1.5rem', textAlign: 'center' }}>
    ...
    return (<div className="glass-card" style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: '650px', position: 'relative' }}>
      
      {/* Fallback mock template banner */}
      {isFallbackMock && (<div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <input type="range" min="0.5" max="3" step="0.25" value={speed} onChange={(e) => setSpeed(parseFloat(e.target.value))} style={{ width: '80px', accentColor: 'var(--primary-neon)' }}/>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'rgba(0,0,0,0.3)', borderRadius: '12px', marginBottom: '1rem', border: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', width: '100%', justifyContent: 'space-between' }}>
          <div style={{ textAlign: 'right', flex: 1 }}>
            ...
            <div style={{ fontSize: '1.1rem', fontWeight: 800, color: '#fff' }}>{currentHomeTeam?.name || 'GoalChain FC'}</div>
          </div>
      ...
      {showControls && (<div style={{ display: 'flex', gap: '8px', marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <button onClick={handleStart} disabled={isRunning || gchBalance < 50} className={isRunning ? 'btn-outline-green' : 'btn-neon-green'} style={{ padding: '8px 16px', fontSize: '0.75rem', fontWeight: 'bold', borderRadius: '8px', opacity: (gchBalance < 50 && !isRunning) ? 0.5 : 1 }}>
            {isRunning ? '⚽ En Juego...' : `▶️ Iniciar (Costo: 50 $GCH)`}
          </button>
          <button onClick={isRunning ? pause : resume} disabled={!isRunning && events.length === 0} className={isRunning ? 'btn-neon-orange' : 'btn-outline-green'} style={{ padding: '8px 16px', fontSize: '0.75rem', fontWeight: 'bold', borderRadius: '8px' }}>
            {isRunning ? '⏸️ Pausar' : '▶️ Continuar'}
          </button>
          <button onClick={() => { stop(); setMatchFinished(false); }} disabled={events.length === 0 && !isRunning} className="btn-outline-red" style={{ padding: '8px 16px', fontSize: '0.75rem', fontWeight: 'bold', borderRadius: '8px' }}>
            ⏹️ Detener
          </button>
          {gchBalance < 50 && !isRunning && (<span style={{ fontSize: '0.72rem', color: 'var(--accent-red)', fontWeight: 600 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '8px' }}>
          ...
          <button onClick={() => handleTacticChange('defense_total')} disabled={!isRunning} style={{
          ...
          <button onClick={() => handleTacticChange('attack_total')} disabled={!isRunning} style={{
          ...
          <button onClick={() => handleTacticChange('normal')} disabled={!isRunning} style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem', flex: 1 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '0.5rem', background: 'rgba(0,0,0,0.2)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.03)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '350px', overflowY: 'auto', paddingRight: '4px' }}>
            ...
            return (<div key={p.playerId} style={{
                    display: 'flex',
                    alignItems: 'center',
                  ...
                  <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                    <button onClick={() => handleEnergyBoost(p.playerId)} disabled={!isRunning || gchBalance < 15 || staminaPct > 80} title="GCH Energy Boost: Recupera 45% stamina (Costo: 15 $GCH)" style={{
                    padding: '4px 8px',
                    ...
                    <button onClick={() => setSelectedSubOutId(p.playerId)} disabled={!isRunning || reserves.length === 0} title="Sustitución de Emergencia: Reemplaza por un suplente fresco (Gratis)" style={{
                    padding: '4px 8px',
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px', padding: '0.5rem', background: 'rgba(0,0,0,0.2)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.03)', maxHeight: '350px' }}>
            {commentary.length === 0 ? (<div style={{ textAlign: 'center', color: '#64748b', padding: '2rem', fontSize: '0.85rem', margin: 'auto' }}>
                {isRunning ? '⚽ Partido en curso... generando comentarios' : '▶️ Inicia el partido para ver la narración táctica en vivo'}
              </div>) : (commentary.map((line) => {
      ...
      <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem', padding: '0.75rem', background: 'rgba(0,0,0,0.2)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.03)' }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
            {currentAwayTeam?.players.slice(0, 11).map(p => (<span key={p.playerId} style={{
                fontSize: '0.6rem',
                padding: '2px 6px',
      ...
      {selectedSubOutId && (<div style={{
          ...
          <div className="glass-card" style={{ width: '100%', maxWidth: '400px', background: 'rgba(10,20,38,0.95)', border: '1px solid rgba(20,241,149,0.3)', borderRadius: '20px', padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '200px', overflowY: 'auto', marginBottom: '1.5rem' }}>
              {reserves.map((p) => (<button key={p.playerId} onClick={() => handleSubSelect(p)} style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                    alignItems: 'center',
              ...
              {reserves.length === 0 && (<div style={{ color: '#64748b', fontSize: '0.75rem', textAlign: 'center', padding: '1rem' }}>
                  No tienes suplentes disponibles.
                </div>)}
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button className="btn-outline-red" onClick={() => setSelectedSubOutId(null)} style={{ padding: '6px 12px', fontSize: '0.7rem', borderRadius: '6px' }}>
                Cancelar
              </button>
            </div>
          </div>
      ...
      {showResultModal && state && (<div style={{
                position: 'absolute',
                inset: 0,
          ...
          <div className="glass-card" style={{
            ...
            <button className="btn-neon-green" onClick={() => setShowResultModal(false)} style={{ width: '100%', padding: '10px', fontSize: '0.8rem', borderRadius: '10px', fontWeight: 'bold' }}>
              Entendido 🎮
            </button>
          </div>
        </div>)}

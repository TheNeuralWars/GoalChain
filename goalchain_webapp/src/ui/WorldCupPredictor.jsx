...
export function WorldCupPredictor() {
    ...
    const [darkHorsePick, setDarkHorsePick] = useState('');
    ...
    useEffect(() => {
        ...
        const savedPickem = localStorage.getItem('goalchain_pickem');
        if (savedPickem) {
            try {
                const parsed = JSON.parse(savedPickem);
                if (parsed.champion)
                    setChampionPick(parsed.champion);
                if (parsed.darkHorse)
                    setDarkHorsePick(parsed.darkHorse);
                if (parsed.topScorer)
                    setTopScorerPick(parsed.topScorer);
            }
            catch (e) { /* ignore */ }
        }
    }, []);
    ...
    const handleSavePickem = () => {
        ...
        localStorage.setItem('goalchain_pickem', JSON.stringify(data));
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', textAlign: 'left' }}>
      
      {/* Sub-tab Navigation */}
      <div style={{ display: 'flex', gap: '8px', borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '8px', overflowX: 'auto' }}>
        {['simulator', 'h2h', 'standings', 'pickem'].map((tab) => (<button key={tab} onClick={() => setActiveSubTab(tab)} style={{
      ...
      {/* 1. SIMULATOR VIEW */}
      {activeSubTab === 'simulator' && (<div className="portal-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '30px', justifyContent: 'space-around' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', padding: '6px', borderRadius: '8px', minHeight: '68px', justifyContent: 'center' }}>
                {r16.m1_w ? (<button onClick={() => setQf(p => ({ ...p, q1_w: r16.m1_w }))} style={{ border: 'none', background: qf.q1_w === r16.m1_w ? 'rgba(20,241,149,0.15)' : 'transparent', color: qf.q1_w === r16.m1_w ? 'var(--primary-neon)' : '#fff', cursor: 'pointer', display: 'flex', gap: '8px', padding: '6px', fontSize: '0.75rem', borderRadius: '4px', fontWeight: qf.q1_w === r16.m1_w ? 'bold' : 'normal' }}>
                    <span>{getEmoji(r16.m1_w)}</span> {r16.m1_w}
                  </button>) : <span style={{ fontSize: '0.7rem', color: '#475569', padding: '6px' }}>Esperando 1/8...</span>}
                
                {r16.m2_w ? (<button onClick={() => setQf(p => ({ ...p, q1_w: r16.m2_w }))} style={{ border: 'none', background: qf.q1_w === r16.m2_w ? 'rgba(20,241,149,0.15)' : 'transparent', color: qf.q1_w === r16.m2_w ? 'var(--primary-neon)' : '#fff', cursor: 'pointer', display: 'flex', gap: '8px', padding: '6px', fontSize: '0.75rem', borderRadius: '4px', fontWeight: qf.q1_w === r16.m2_w ? 'bold' : 'normal' }}>
              ...
              {/* QF Match 2 */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', padding: '6px', borderRadius: '8px', minHeight: '68px', justifyContent: 'center' }}>
                {r16.m3_w ? (<button onClick={() => setQf(p => ({ ...p, q2_w: r16.m3_w }))} style={{ border: 'none', background: qf.q2_w === r16.m3_w ? 'rgba(20,241,149,0.15)' : 'transparent', color: qf.q2_w === r16.m3_w ? 'var(--primary-neon)' : '#fff', cursor: 'pointer', display: 'flex', gap: '8px', padding: '6px', fontSize: '0.75rem', borderRadius: '4px', fontWeight: qf.q2_w === r16.m3_w ? 'bold' : 'normal' }}>
                    <span>{getEmoji(r16.m3_w)}</span> {r16.m3_w}
                  </button>) : <span style={{ fontSize: '0.7rem', color: '#475569', padding: '6px' }}>Esperando 1/8...</span>}
                
                {r16.m4_w ? (<button onClick={() => setQf(p => ({ ...p, q2_w: r16.m4_w }))} style={{ border: 'none', background: qf.q2_w === r16.m4_w ? 'rgba(20,241,149,0.15)' : 'transparent', color: qf.q2_w === r16.m4_w ? 'var(--primary-neon)' : '#fff', cursor: 'pointer', display: 'flex', gap: '8px', padding: '6px', fontSize: '0.75rem', borderRadius: '4px', fontWeight: qf.q2_w === r16.m4_w ? 'bold' : 'normal' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '40px', justifyContent: 'space-around' }}>
              ...
              {/* SF Match 1 */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', padding: '6px', borderRadius: '8px', minHeight: '68px', justifyContent: 'center' }}>
                {qf.q1_w ? (<button onClick={() => setSf(p => ({ ...p, s1_w: qf.q1_w }))} style={{ border: 'none', background: sf.s1_w === qf.q1_w ? 'rgba(20,241,149,0.15)' : 'transparent', color: sf.s1_w === qf.q1_w ? 'var(--primary-neon)' : '#fff', cursor: 'pointer', display: 'flex', gap: '8px', padding: '6px', fontSize: '0.75rem', borderRadius: '4px', fontWeight: sf.s1_w === qf.q1_w ? 'bold' : 'normal' }}>
                    <span>{getEmoji(qf.q1_w)}</span> {qf.q1_w}
                  </button>) : <span style={{ fontSize: '0.7rem', color: '#475569', padding: '6px' }}>Esperando QF...</span>}
                
                {qf.q2_w ? (<button onClick={() => setSf(p => ({ ...p, s1_w: qf.q2_w }))} style={{ border: 'none', background: sf.s1_w === qf.q2_w ? 'rgba(20,241,149,0.15)' : 'transparent', color: sf.s1_w === qf.q2_w ? 'var(--primary-neon)' : '#fff', cursor: 'pointer', display: 'flex', gap: '8px', padding: '6px', fontSize: '0.75rem', borderRadius: '4px', fontWeight: sf.s1_w === qf.q2_w ? 'bold' : 'normal' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', justifyContent: 'center', alignItems: 'center' }}>
              ...
              {sf.s1_w ? (<div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center', width: '100%' }}>
                  <button onClick={() => setChampion(sf.s1_w)} style={{
                  ...
                  {champion === sf.s1_w && (<span style={{ fontSize: '0.65rem', color: 'var(--primary-neon)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>
                      👑 Campeón Elegido
                    </span>)}
                </div>) : (<div style={{ border: '1px dashed rgba(255,255,255,0.1)', padding: '20px', borderRadius: '12px', color: '#475569', fontSize: '0.75rem', textAlign: 'center' }}>
      ...
      {activeSubTab === 'h2h' && (<div className="portal-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
            ...
            <div className="glass-card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '8px' }}>
                <div>
                  ...
                  <select value={teamA} onChange={(e) => setTeamA(e.target.value)} style={{ width: '100%', background: '#070f1e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', padding: '6px', color: '#fff', fontSize: '0.75rem', outline: 'none' }}>
                    {WC_TEAMS.map(t => (<option key={t.name} value={t.name} disabled={t.name === teamB}>{t.emoji} {t.name}</option>))}
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: '0.68rem', color: '#94a3b8', display: 'block', marginBottom: '4px' }}>Equipo B</label>
                  <select value={teamB} onChange={(e) => setTeamB(e.target.value)} style={{ width: '100%', background: '#070f1e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', padding: '6px', color: '#fff', fontSize: '0.75rem', outline: 'none' }}>
                    {WC_TEAMS.map(t => (<option key={t.name} value={t.name} disabled={t.name === teamA}>{t.emoji} {t.name}</option>))}
                  </select>
                </div>
              </div>

              {h2hResult && (<div style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '12px', padding: '12px', textAlign: 'center', marginTop: '10px' }}>
            ...
            <div className="glass-card" style={{ padding: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {WC_TEAMS.slice(0, 10).map((t, idx) => (<div key={t.name} style={{ fontSize: '0.72rem' }}>
      ...
      {activeSubTab === 'standings' && (<div className="portal-fade-in" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
      ...
      {/* 4. PICK'EM VIEW */}
      {activeSubTab === 'pickem' && (<div className="portal-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div className="glass-card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <span style={{ fontSize: '0.72rem', color: '#94a3b8', display: 'block', marginTop: '-4px' }}>
              Guarda tus predicciones personales del torneo. Una vez que se resuelva la Copa del Mundo GoalChain 2026, recibirás reputación y multiplicadores de staking en base a tus aciertos.
            </span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div>
                ...
                <select value={championPick} onChange={(e) => setChampionPick(e.target.value)} style={{ width: '100%', maxWidth: '300px', background: '#070f1e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '8px', color: '#fff', fontSize: '0.78rem', outline: 'none' }}>
                  <option value="">Selecciona Campeón...</option>
                  {WC_TEAMS.map(t => (<option key={t.name} value={t.name}>{t.emoji} {t.name}</option>))}
                </select>
              </div>

              <div>
                <label style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#fff', display: 'block', marginBottom: '6px' }}>🐎 Caballo Negro / Revelación</label>
                <select value={darkHorsePick} onChange={(e) => setDarkHorsePick(e.target.value)} style={{ width: '100%', maxWidth: '300px', background: '#070f1e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '8px', color: '#fff', fontSize: '0.78rem', outline: 'none' }}>
              ...
              <div>
                ...
                <input type="text" value={topScorerPick} onChange={(e) => setTopScorerPick(e.target.value)} placeholder="Ej: Lionel Satoshi o Kylian Vitalik" style={{ width: '100%', maxWidth: '300px', background: '#070f1e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '8px 12px', color: '#fff', fontSize: '0.78rem', outline: 'none' }}/>
              </div>

            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '8px' }}>
              <button onClick={handleSavePickem} className="btn-neon-green" style={{ padding: '8px 16px', borderRadius: '8px', fontSize: '0.78rem', fontWeight: 'bold', cursor: 'pointer' }}>
                💾 Guardar Predicciones
              </button>
              
              {savedSuccess && (<span style={{ fontSize: '0.75rem', color: 'var(--primary-neon)', fontWeight: 'bold', animation: 'fadeIn 0.3s ease-out' }}>

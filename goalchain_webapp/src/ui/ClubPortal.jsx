...
export function ClubPortal() {
    ...
    return (<div className="play-page play-page--portal">
      <div className="portal-header glass-card">
        ...
        <div className="portal-tabs" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '8px' }}>
          {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveSubTab(tab.id)} className={`portal-tab-btn portal-tab-btn--club ${activeSubTab === tab.id ? 'portal-tab-btn--active' : ''}`}>
      ...
      <div className="portal-content-wrapper">
        {activeSubTab === 'squad' && (<div className="portal-fade-in">
            <SquadGallery />
          </div>)}
        {activeSubTab === 'arena' && (<div className="portal-fade-in">
            <MatchSimulator />
          </div>)}
        {activeSubTab === 'market' && (<div className="portal-fade-in">
            <NFTMarketplace />
          </div>)}
        {activeSubTab === 'coach' && (<div className="portal-fade-in">
            <AICoach />
          </div>)}
        {activeSubTab === 'profile' && (<div className="portal-fade-in">
            {isLoggedIn ? (<UserProfile username={user?.username}/>) : (<div className="registration-wrapper glass-card">

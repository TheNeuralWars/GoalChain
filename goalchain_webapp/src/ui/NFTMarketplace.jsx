...
export function NFTMarketplace() {
    ...
    const handleBuy = async (player, mode) => {
    ...
    return (<div className="glass-card" style={{ padding: '1.25rem', textAlign: 'left' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <h2 className="text-neon-purple" style={{ margin: 0, borderBottom: 'none', paddingBottom: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <SimulationBadge />
            {treasuryAddress === null && (<span style={{ fontSize: '0.6rem', color: 'var(--accent-red)', fontWeight: 600 }}>
                ⛔ SOL OFFLINE
              </span>)}
          </h2>
      ...
      <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '10px', marginBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        {filters.map(filter => (<button key={filter} onClick={() => setActiveFilter(filter)} style={{
                padding: '6px 14px',
                borderRadius: '20px',
                fontSize: '0.75rem',
                fontWeight: 'bold',
      ...
      {/* Grid of Listings */}
      {filteredListings.length === 0 ? (<div style={{ textAlign: 'center', padding: '60px' }}>
          ...
          {filteredListings.map(player => (<LayeredNftCard key={player.id} player={player} isMarketplace={true} onBuyCash={() => handleBuy(player, 'cash')} onBuySol={() => handleBuy(player, 'solana')} isSolOffline={!treasuryAddress}/>))}
        </div>)}
    </div>);
}

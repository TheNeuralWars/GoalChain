...
export const TradingTerminal = () => {
    ...
    const [selectedPair, setSelectedPair] = useState('Argentina (ARG-PERP)');
    const [position, setPosition] = useState('Long');
    const [leverage, setLeverage] = useState(1);
    // Real-time simulated price history for the oracle chart
    const [priceHistory, setPriceHistory] = useState([100, 101.5, 99.8, 102.3, 101.0, 103.5, 102.8, 105.4, 104.2, 106.5]);
    // Vibe Bots States
    const [sentiment, setSentiment] = useState(50);
    const [botLogs, setBotLogs] = useState([]);
    const [toroState, setToroState] = useState({
        isEnabled: false,
        balance: 1000,
        totalProfit: 0,
        activePosition: null
    });
    const [osoState, setOsoState] = useState({
        isEnabled: false,
        balance: 1000,
        totalProfit: 0,
        activePosition: null
    ...
    useEffect(() => {
        ...
        return () => window.removeEventListener('goalchain-event', handleEvent);
    }, []);
    ...
    const toroUnrealizedPnl = useMemo(() => {
        if (!toroState.activePosition || !currentPrice)
            return 0;
        const entry = toroState.activePosition.entryPrice;
        const size = toroState.activePosition.size;
        const lev = toroState.activePosition.leverage;
    ...
    const osoUnrealizedPnl = useMemo(() => {
        if (!osoState.activePosition || !currentPrice)
            return 0;
        const entry = osoState.activePosition.entryPrice;
        const size = osoState.activePosition.size;
        const lev = osoState.activePosition.leverage;
    ...
    const renderChart = () => (<div style={{
            ...
            <div style={{ position: 'relative', height: '110px', marginTop: '1rem', marginBottom: '0.5rem' }}>
                <svg width="100%" height="100%" viewBox="0 0 300 120" preserveAspectRatio="none" style={{ overflow: 'visible' }}>
                    ...
                    {/* Glowing Active Dot */}
                    {points.length > 0 && (<>
                            <circle cx={points[points.length - 1].x} cy={points[points.length - 1].y} r="4.5" fill={activeColor} style={{ filter: `drop-shadow(0 0 5px ${activeColor})` }}/>
                            <circle cx={points[points.length - 1].x} cy={points[points.length - 1].y} r="9" fill="none" stroke={activeColor} strokeWidth="1.5" opacity="0.6">
    ...
    return (<div className="glass-card trading-terminal" style={{ textAlign: 'left' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px', marginBottom: '1rem' }}>
                ...
                <div className="terminal-tabs" style={{ marginBottom: 0, paddingBottom: 0, borderBottom: 'none' }}>
                    <button onClick={() => setActiveTab('manual')} className={`terminal-tab-btn ${activeTab === 'manual' ? 'active' : ''}`}>
                        🎮 Manual
                    </button>
                    <button onClick={() => setActiveTab('vibe')} className={`terminal-tab-btn ${activeTab === 'vibe' ? 'active' : ''}`}>
                        🤖 Vibe Bots
                    </button>
                </div>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        <div>
                            ...
                            <select value={selectedPair} onChange={(e) => setSelectedPair(e.target.value)} className="form-select" style={{ marginTop: '0.5rem' }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <button onClick={() => setPosition('Long')} className={position === 'Long' ? 'btn-neon-green' : 'btn-outline-green'} style={{ flex: 1, padding: '0.5rem 1rem' }}>
                                    Long (Comprar)
                                </button>
                                <button onClick={() => setPosition('Short')} className={position === 'Short' ? 'btn-neon-red' : 'btn-outline-red'} style={{ flex: 1, padding: '0.5rem 1rem' }}>
                                    Short (Vender)
                                </button>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="range" min="1" max="10" step="1" value={leverage} onChange={(e) => setLeverage(parseInt(e.target.value))} className="premium-slider"/>
                        </div>

                        <button onClick={handleExecute} className={position === 'Long' ? 'btn-neon-green' : 'btn-neon-red'} style={{ width: '100%', padding: '0.8rem', fontSize: '0.9rem', marginTop: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem' }}>
                            ...
                            <div className={`bot-switch-container ${toroState.isEnabled ? 'active-toro' : ''}`}>
                                <div className="switch-label">
                                    ...
                                    <div style={{ fontSize: '0.7rem', marginTop: '4px' }}>
                                        {toroState.activePosition ? (<span style={{ color: 'var(--primary-neon)', fontWeight: 700 }}>
                                                🟢 LONG Activo @ ${toroState.activePosition.entryPrice.toFixed(2)} ({toroUnrealizedPnl >= 0 ? '+' : ''}${toroUnrealizedPnl.toFixed(2)})
                                            </span>) : (<span style={{ color: '#64748b' }}>⚪ Idle</span>)}
                                    </div>
                                </div>
                                <label className="switch">
                                    <input type="checkbox" checked={toroState.isEnabled} onChange={(e) => setToroState(prev => ({
                ...prev,
                ...
                activePosition: e.target.checked ? prev.activePosition : null
                            ...
                            <div className={`bot-switch-container ${osoState.isEnabled ? 'active-oso' : ''}`}>
                                <div className="switch-label">
                                    ...
                                    <div style={{ fontSize: '0.7rem', marginTop: '4px' }}>
                                        {osoState.activePosition ? (<span style={{ color: 'var(--accent-red)', fontWeight: 700 }}>
                                                🔴 SHORT Activo @ ${osoState.activePosition.entryPrice.toFixed(2)} ({osoUnrealizedPnl >= 0 ? '+' : ''}${osoUnrealizedPnl.toFixed(2)})
                                            </span>) : (<span style={{ color: '#64748b' }}>⚪ Idle</span>)}
                                    </div>
                                </div>
                                <label className="switch">
                                    <input type="checkbox" checked={osoState.isEnabled} onChange={(e) => setOsoState(prev => ({
                ...prev,
                isEnabled: e.target.checked,
                activePosition: e.target.checked ? prev.activePosition : null
            }))}/>
                ...
                gap: '8px'
                            ...
                            <div className="vibe-ledger">
                                {botLogs.length > 0 ? (botLogs.map(log => (<div key={log.id} className="vibe-ledger-row">
                                            ...
                                            <div style={{ textAlign: 'right', display: 'flex', gap: '8px', alignItems: 'center' }}>
                                                ...
                                                {log.pnl !== undefined && (<span style={{ fontWeight: 800, color: log.pnl >= 0 ? 'var(--primary-neon)' : 'var(--accent-red)' }}>
                                                        {log.pnl >= 0 ? '+' : ''}${log.pnl}
                                                    </span>)}
                                            </div>
                                        </div>))) : (<div style={{ color: '#475569', textAlign: 'center', padding: '2rem 0', fontSize: '0.72rem', fontStyle: 'italic' }}>

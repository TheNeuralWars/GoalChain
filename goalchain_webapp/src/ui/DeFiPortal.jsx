...
export function DeFiPortal() {
    const [activeSubTab, setActiveSubTab] = useState('trading');
    const tabs = [
    ...
    return (<div className="play-page play-page--portal">
      <div className="portal-header glass-card">
        ...
        <div className="portal-tabs portal-tabs--two-col">
          {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveSubTab(tab.id)} className={`portal-tab-btn portal-tab-btn--defi ${activeSubTab === tab.id ? 'portal-tab-btn--active' : ''}`}>
      ...
      <div className="portal-content-wrapper">
        {activeSubTab === 'trading' && (<div className="portal-fade-in">
            <Suspense fallback={<div style={{ color: '#64748b', padding: '2rem', textAlign: 'center' }}>Cargando terminal...</div>}>
              <TradingTerminal />
            </Suspense>
          </div>)}
        {activeSubTab === 'vaults' && (<div className="portal-fade-in">

...
export const AICommentator = () => {
    const wsUrl = import.meta.env?.VITE_STREAMING_WS_URL;
    const wsEnabled = Boolean(wsUrl);
    const [loadingPhase, setLoadingPhase] = useState('downloading');
    ...
    const broadcastCommentary = (text, eventType, emotion, duration) => {
        if (!wsEnabled)
            return;
    ...
    useEffect(() => {
        ...
        const updateVoices = () => {
            if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
                ...
                const defaultVoice = availableVoices.find(voice => voice.lang.startsWith('es') || voice.lang.includes('ES')) || availableVoices.find(voice => voice.lang.startsWith('en') || voice.lang.includes('EN')) || availableVoices[0];
    ...
    useEffect(() => {
        if (loadingPhase === 'active') {
            const welcomeMsg = "🎙️ ¡Enzo Bit conectado! Listo para narrar la acción de GoalChain World Cup 2026 en tiempo real.";
            speak(welcomeMsg);
    ...
    useEffect(() => {
        ...
        const handleEvent = (e) => {
            ...
            const goalPhrases = [
                ...
                `🔥 ¡Se mueve el marcador! ${eventData.message}. ¡Los traders de GoalChain están enloqueciendo!`,
        ...
        window.addEventListener('goalchain-event', handleEvent);
        return () => {
            window.removeEventListener('goalchain-event', handleEvent);
        };
    }, [loadingPhase, isMuted, selectedVoice, voices]);
    if (loadingPhase === 'downloading') {
        return (<div className="glass-card" style={{ padding: '1.5rem', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
    ...
    if (loadingPhase === 'compiling') {
        return (<div className="glass-card" style={{ padding: '1.5rem', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center', justifyContent: 'center', minHeight: '135px' }}>
    ...
    return (<div className="glass-card AICommentator" style={{ textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            color: '#ffffff'
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div title={wsError ?? undefined} style={{
            display: 'flex',
            alignItems: 'center',
            gap: '6px',
            ...
            : wsStatus === 'connected'
                ...
                : wsStatus === 'connecting'
                    ...
                    {voices.length > 0 && (<select value={selectedVoice} onChange={(e) => setSelectedVoice(e.target.value)} className="form-select" style={{
                padding: '4px 8px',
                fontSize: '0.75rem',
            ...
            <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'flex-start' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                    <svg width="75" height="75" viewBox="0 0 100 100" style={{ overflow: 'visible' }}>
                        ...
                        {isSpeaking && (<circle cx="50" cy="50" r="40" fill="url(#glow)"/>)}
                        ...
                        <circle cx="50" cy="10" r="6" fill={isSpeaking ? "var(--primary-neon)" : "var(--secondary-neon)"} style={{
            ...
            transition: 'all 0.3s ease'
                        ...
                        <rect x="38" y="40" width="24" height="6" rx="3" fill={isSpeaking ? "var(--primary-neon)" : "var(--secondary-neon)"} style={{
            filter: isSpeaking
            ...
            transition: 'all 0.2s ease',
                        ...
                        <path d="M 44 58 Q 50 63 56 58" fill="none" stroke={isSpeaking ? "var(--primary-neon)" : "rgba(255, 255, 255, 0.3)"} strokeWidth="2.5" strokeLinecap="round" style={{
            ...
            boxShadow: isSpeaking ? '0 0 15px rgba(20, 241, 149, 0.08)' : 'none'
                        ...
                        {commentaryHistory.length > 0 ? (<p style={{ margin: 0, fontStyle: isSpeaking ? 'normal' : 'italic' }}>
                                {commentaryHistory[0].text}
                            </p>) : (<p style={{ margin: 0, color: '#64748b' }}>Esperando eventos en vivo de Solana (Helius)...</p>)}
                    </div>
                </div>
            </div>

            {/* Commentary History */}
            {commentaryHistory.length > 1 && (<div style={{ display: 'flex', flexDirection: 'column', gap: '8px', borderTop: '1px solid rgba(255, 255, 255, 0.05)', paddingTop: '12px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '120px', overflowY: 'auto', paddingRight: '4px' }}>
                        {commentaryHistory.slice(1).map(item => (<div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', color: '#94a3b8', gap: '12px', background: 'rgba(255, 255, 255, 0.015)', padding: '6px 10px', borderRadius: '8px', border: '1px solid rgba(255, 255, 255, 0.02)' }}>
            ...
            <div style={{ marginTop: '16px', borderTop: '1px solid rgba(255, 255, 255, 0.05)', paddingTop: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                    <input type="text" placeholder="Ej. ¿Qué probabilidad de gol tiene Lionel Satoshi?" value={noahQuery} onChange={(e) => setNoahQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && queryNoahAi()} style={{
            flex: 1,
            ...
            outline: 'none'
                    ...
                    <button onClick={queryNoahAi} disabled={isQueryingNoah} style={{
            background: '#14f195',
            color: 'black',
            border: 'none',
            borderRadius: '8px',

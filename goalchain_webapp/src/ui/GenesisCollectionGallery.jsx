...
export const GenesisCollectionGallery = () => {
    ...
    const filtered = useMemo(() => {
        let list = [...players];
        if (rarity !== 'all')
            list = list.filter((p) => p.rarity === rarity);
        if (showOnlyFavs)
    ...
    return (<div style={{ width: '100%', maxWidth: 1400, margin: '0 auto', padding: '0 20px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 24, alignItems: 'center' }}>
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar jugador o país…" style={{
            flex: '1 1 250px',
            padding: '10px 14px',
            borderRadius: 8,
        ...
        <select value={rarity} onChange={(e) => setRarity(e.target.value)} style={{
            padding: '10px 14px',
            borderRadius: 8,
            background: '#0f172a',
            color: '#fff',
        ...
        <button onClick={() => setShowOnlyFavs(!showOnlyFavs)} style={{
            padding: '10px 16px',
            borderRadius: 8,
      ...
      <div style={{
        ...
        {filtered.map((p) => (<LayeredNftCard key={p.id} player={p} isFav={favorites.includes(p.id)} onToggleFav={toggleFav}/>))}
      </div>
    </div>);
};

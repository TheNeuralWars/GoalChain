...
const ACCOUNT_STYLES = {
    ...
    GoalChainSol: { accent: '#9945ff', label: 'GoalChain', platforms: ['youtube', 'instagram'] },
};
...
// ── Video Card ────────────────────────────────────────────────────────────────
function VideoCard({ run, apiBase, onComment, }) {
    const acct = ACCOUNT_STYLES[run.account_name] ?? ACCOUNT_STYLES.GoalChainSol;
    ...
    return (<article style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {acct.platforms.map(p => {
            ...
            return (<span key={p} title={slotTime ? `${pm.name}: ${formatScheduledAt(slotTime)}` : pm.name} style={{
      ...
      <div style={{
        ...
        {/* ── LEFT: Video Player ── */}
        {(run.video_url || run.image_url) && (<div style={{
                position: 'relative',
                background: '#000',
            ...
            {run.video_url ? (<video src={run.video_url} autoPlay controls loop muted playsInline preload="auto" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}/>) : (<img src={run.image_url} alt="frame" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}/>)}
          </div>)}
        ...
        <div style={{ padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: '10px', overflow: 'hidden' }}>
          ...
          {scheduledTime && (<div style={{
              ...
              {Object.keys(platformSlots).length > 1 && (<div style={{ marginTop: '4px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {Object.entries(platformSlots).map(([svc, t]) => {
                    const pm = PLATFORM_META[svc];
                    return pm ? (<span key={svc} style={{ fontSize: '0.6rem', color: pm.color }}>
          ...
          {/* Narrative angle */}
          {run.narrative_angle && (<div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.45)', fontStyle: 'italic', lineHeight: 1.4 }}>
              🎯 {run.narrative_angle}
            </div>)}
          ...
          {run.post_text && (<div>
              ...
              <div style={{
                ...
                {run.post_text}
                {!postExpanded && run.post_text.length > 200 && (<div style={{
                    position: 'absolute', bottom: 0, left: 0, right: 0, height: '28px',
                    background: 'linear-gradient(transparent, rgba(10,10,22,0.95))',
                }}/>)}
              </div>
              {run.post_text.length > 200 && (<button onClick={() => setPostExpanded(v => !v)} style={{
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap', marginTop: 'auto' }}>
            ...
            {run.buffer_post_ids && run.buffer_post_ids.length > 0 && (<span style={{ fontSize: '0.62rem', color: 'rgba(255,255,255,0.3)' }}>
                📤 Buffer IDs: {run.buffer_post_ids.join(', ')}
              </span>)}
            {run._reconstructed && (<span style={{ fontSize: '0.58rem', color: 'rgba(255,255,255,0.2)', fontStyle: 'italic' }}>
                archivo recuperado
              </span>)}
          </div>

          {/* Expand toggle for prompts */}
          {(run.image_prompt || run.video_prompt) && (<button onClick={() => setShowPrompts(v => !v)} style={{
          ...
          {showPrompts && (<div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {run.image_prompt && (<div>
          ...
          {/* Error detail */}
          {isFailed && run.error_message && (<div style={{
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
            {run.comments && run.comments.length > 0 && (<div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '8px' }}>
                {run.comments.map((c, i) => (<div key={i} style={{
            ...
            <form onSubmit={handleComment} style={{ display: 'flex', gap: '6px' }}>
              <input type="text" placeholder={isPlanned ? 'Opina sobre este plan...' : 'Feedback para próximos videos...'} value={commentText} onChange={e => setCommentText(e.target.value)} style={{
...
function PlannedCard({ run, onEdit, onTrigger, onDelete, onComment, daemonOnline, }) {
    const acct = ACCOUNT_STYLES[run.account_name] ?? ACCOUNT_STYLES.GoalChainSol;
    const [commentText, setCommentText] = useState('');
    const handleComment = async (e) => {
        e.preventDefault();
    ...
    return (<div style={{
      ...
      {run.narrative_angle && (<p style={{ margin: 0, fontSize: '0.68rem', color: 'rgba(255,255,255,0.4)', fontStyle: 'italic' }}>
          🎯 {run.narrative_angle}
        </p>)}
      <form onSubmit={handleComment} style={{ display: 'flex', gap: '6px' }}>
        <input type="text" placeholder="Opina sobre este plan..." value={commentText} onChange={e => setCommentText(e.target.value)} style={{ flex: 1, padding: '4px 8px', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.06)', color: '#fff', borderRadius: '6px', fontSize: '0.68rem', outline: 'none' }}/>
...
function EditModal({ run, onSave, onClose }) {
    ...
    return (<div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="Tema" style={{ padding: '8px 12px', background: '#040408', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', borderRadius: '8px', fontSize: '0.85rem', outline: 'none' }}/>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          <textarea value={imagePrompt} onChange={e => setImagePrompt(e.target.value)} rows={3} placeholder="Image prompt" style={{ padding: '7px 10px', background: '#040408', border: '1px solid rgba(255,255,255,0.08)', color: '#a0aec0', borderRadius: '8px', fontSize: '0.72rem', fontFamily: 'monospace', resize: 'vertical', outline: 'none' }}/>
          <textarea value={videoPrompt} onChange={e => setVideoPrompt(e.target.value)} rows={3} placeholder="Video prompt" style={{ padding: '7px 10px', background: '#040408', border: '1px solid rgba(255,255,255,0.08)', color: '#a0aec0', borderRadius: '8px', fontSize: '0.72rem', fontFamily: 'monospace', resize: 'vertical', outline: 'none' }}/>
        </div>
        <textarea value={postText} onChange={e => setPostText(e.target.value)} rows={4} placeholder="Copy del pie de video" style={{ padding: '7px 10px', background: '#040408', border: '1px solid rgba(255,255,255,0.08)', color: '#cbd5e1', borderRadius: '8px', fontSize: '0.78rem', lineHeight: 1.5, resize: 'vertical', outline: 'none' }}/>
...
export function MarketingControlCenter() {
    ...
    const [daemon, setDaemon] = useState({ status: 'offline', is_online: false });
    const [filter, setFilter] = useState('all');
    const [targetAccount, setTargetAccount] = useState('GoalChainSol');
    const [customTopic, setCustomTopic] = useState('');
    const [triggering, setTriggering] = useState(false);
    const [researching, setResearching] = useState(false);
    const [logs, setLogs] = useState('');
    const [activeLogRunId, setActiveLogRunId] = useState(null);
    ...
    const handleTriggerPlanned = async (runId) => {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', textAlign: 'left', width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          {scheduledCount > 0 && (<span style={{ fontSize: '0.62rem', color: 'rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.04)', padding: '2px 7px', borderRadius: '5px' }}>
              {scheduledCount} programados en Buffer
            </span>)}
        </div>
        {/* Filter tabs */}
        <div style={{ display: 'flex', gap: '5px' }}>
          {['all', 'planned', 'GoalChainSol', 'NicoPezDorado'].map(f => (<button key={f} id={`filter-${f}`} onClick={() => setFilter(f)} style={{
      ...
      <div style={{ background: 'rgba(10,10,22,0.7)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '14px 16px' }}>
        <form onSubmit={handleTrigger} style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          <select id="target-account-select" value={targetAccount} onChange={e => setTargetAccount(e.target.value)} style={{ padding: '7px 10px', fontSize: '0.82rem', borderRadius: '8px', flex: '0 0 auto', background: '#0a0a1a', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', cursor: 'pointer' }}>
            <option value="GoalChainSol">GoalChainSol</option>
            <option value="NicoPezDorado">NicoPezDorado</option>
            <option value="both">Ambas</option>
          </select>
          <input id="custom-topic-input" type="text" placeholder="Tema específico (vacío = Hermes decide)" value={customTopic} onChange={e => setCustomTopic(e.target.value)} style={{ flex: 1, minWidth: '180px', padding: '7px 12px', background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.08)', color: '#fff', borderRadius: '8px', fontSize: '0.82rem', outline: 'none' }}/>
          <button id="wake-hermes-btn" type="submit" disabled={triggering || !daemon.is_online} style={{ padding: '7px 16px', borderRadius: '8px', fontSize: '0.82rem', fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap', background: 'rgba(153,69,255,0.2)', border: '1px solid rgba(153,69,255,0.5)', color: '#9945ff', opacity: (!daemon.is_online || triggering) ? 0.5 : 1 }}>
            {triggering ? '...' : '🚀 Generar'}
          </button>
          <button id="fill-queue-btn" type="button" onClick={handleResearch} disabled={researching || !daemon.is_online} style={{ padding: '7px 16px', borderRadius: '8px', fontSize: '0.82rem', fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap', background: 'rgba(20,241,149,0.12)', border: '1px solid rgba(20,241,149,0.35)', color: '#14f195', opacity: (!daemon.is_online || researching) ? 0.5 : 1 }}>
            {researching ? '...' : '🧠 Llenar Cola'}
          </button>
          <button id="toggle-console-btn" type="button" onClick={() => setShowConsole(v => !v)} style={{ padding: '7px 10px', borderRadius: '8px', fontSize: '0.72rem', background: showConsole ? 'rgba(20,241,149,0.1)' : 'rgba(255,255,255,0.04)', border: `1px solid ${showConsole ? 'rgba(20,241,149,0.3)' : 'rgba(255,255,255,0.07)'}`, color: showConsole ? '#14f195' : 'rgba(255,255,255,0.4)', cursor: 'pointer' }}>
            📟
          </button>
        </form>
        {!daemon.is_online && (<p style={{ margin: '8px 0 0 0', fontSize: '0.7rem', color: '#f04040' }}>
            ⚠️ Hermes está durmiendo. Ejecuta <code style={{ fontSize: '0.65rem' }}>pm2 start hermes-video-daemon</code> en el VPS.
          </p>)}
      </div>

      {/* ── CONSOLE ── */}
      {showConsole && (<div style={{ background: 'rgba(4,4,9,0.95)', border: '1px solid rgba(20,241,149,0.2)', borderRadius: '12px', padding: '12px' }}>
      ...
      {/* ── PLANNED QUEUE ── */}
      {(filter === 'all' || filter === 'planned') && plannedRuns.length > 0 && (<section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {plannedRuns.map(run => (<PlannedCard key={run.id} run={run} onEdit={setEditingRun} onTrigger={handleTriggerPlanned} onDelete={handleDelete} onComment={handleComment} daemonOnline={daemon.is_online}/>))}
          </div>
        </section>)}

      {/* ── VIDEO GALLERY ── */}
      {filter !== 'planned' && (<section>
          <h3 style={{ margin: '0 0 0.75rem 0', fontSize: '0.9rem', fontWeight: 800, color: 'rgba(255,255,255,0.6)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            🎬 Galería de Videos
            <span style={{ fontSize: '0.75rem', background: 'rgba(255,255,255,0.07)', padding: '2px 8px', borderRadius: '10px' }}>{historyRuns.length}</span>
          </h3>
          {historyRuns.length === 0 ? (<div style={{ textAlign: 'center', padding: '40px 20px', background: 'rgba(10,10,22,0.5)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              {historyRuns.map(run => (<VideoCard key={run.id} run={run} apiBase={apiBase} onComment={handleComment}/>))}
            </div>)}
        </section>)}

      {/* ── EDIT MODAL ── */}
      {editingRun && (<EditModal run={editingRun} onSave={handleSaveEdit} onClose={() => setEditingRun(null)}/>)}

...
export const LiveEventFeed = () => {
        ...
        const refresh = async () => {
            try {
                ...
                const next = fixtures.slice(0, 5).map((f, idx) => {
                    ...
                    const type = f.status === 'completed' ? 'RESOLVE' : (f.status === 'live' ? 'GOAL' : 'BET');
                    return {
    ...
    return (<div className="live-feed" style={{
            marginTop: '2rem',
            padding: '1.5rem',
            ...
            {error && (<div style={{ color: '#ff9ea8', fontSize: '0.8rem', marginBottom: 10 }}>{error}</div>)}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {events.length === 0 && !error && (<div style={{ fontSize: '0.85rem', opacity: 0.75 }}>Sin eventos recientes on-chain para mostrar.</div>)}

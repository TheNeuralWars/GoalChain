...
import { fetchEconomyMetrics, fetchEconomyHealth } from '../lib/economyClient';
...
export const StakingBurnDashboard = () => {
    const [metrics, setMetrics] = useState(null);
    const [health, setHealth] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [selectedStrategy, setSelectedStrategy] = useState('sentinel');
    ...
    if (loading && !metrics) {
        return (<div className="staking-burn-dashboard" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
    ...
    if (error) {
        return (<div className="glass-card-premium" style={{ textAlign: 'center', padding: '2rem' }}>
    ...
    return (<div className="staking-burn-dashboard" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          {health && (<div className="glass-card-premium health-status-badge" style={{
          ...
          {lastUpdated && (<span style={{ color: '#64748b', fontSize: '0.8rem', fontFamily: 'monospace' }}>
      ...
      <div className="glass-card-flame infinity-burn-card" style={{ position: 'relative', overflow: 'hidden', minHeight: '320px' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0 }}>
          <InfinityFlame intensity={intensityRef.current} width={400} height={280} enableParticles={true} className="flame-layer"/>
        </div>
      ...
      <div className="glass-card-premium" style={{ padding: '1.5rem' }}>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {VAULT_STRATEGIES.map(s => (<VaultStrategyCard key={s.id} strategy={s} isSelected={selectedStrategy === s.id} onSelect={() => setSelectedStrategy(s.id)} onKeyDown={(e) => {
        ...
        {/* Yield Calculator */}
        <YieldCalculator selectedStrategy={selectedStrategy} strategies={VAULT_STRATEGIES} vaultBuybackCoverage={vaultBuybackCoverage} gchPrice={0.001} solPrice={150} onStakeAmountChange={(amount) => { }} onStakeTokenChange={(token) => { }}/>
      </div>

      {/* Health Checks Detail */}
      {health && (<HealthCheckCard checks={health.checks} configDriftReasons={health.config_drift_reasons}/>)}
    </div>);
};

...
export const CreateUser = ({ onUserCreated }) => {
    ...
    if (success) {
        return (<div className="create-user-success">
    ...
    return (<div className="create-user-page">
      {/* Header */}
      ...
      <div className="create-user-card">
        ...
        {/* STEP 1: Wallet */}
        {step === 1 && (<div className="step-content" id="step-wallet">
            ...
            <div className="wallet-connect-area">
              <WalletMultiButton />
              {connected && (<div className="wallet-connected-badge">
                  <span className="dot-green"/>
                  Wallet conectada: <code>{shortWallet}</code>
                </div>)}
            </div>
            <button className="btn-primary" id="step1-next" disabled={!connected} onClick={() => setStep(2)}>
              Continuar →
            </button>
          </div>)}

        {/* STEP 2: Identity */}
        {step === 2 && (<div className="step-content" id="step-identity">
            <div className="step-icon">🎭</div>
            <h2>Tu identidad</h2>

            <div className="form-group">
              <label htmlFor="username-input">Nombre de usuario</label>
              <input id="username-input" type="text" className={`form-input ${usernameError ? 'input-error' : username.length >= 3 ? 'input-ok' : ''}`} placeholder="ej: el_toro_sentimental" value={username} onChange={handleUsernameChange} maxLength={20} autoComplete="off"/>
              {usernameError && <span className="input-error-msg">{usernameError}</span>}
              {!usernameError && username.length >= 3 && (<span className="input-ok-msg">✓ Disponible</span>)}
            </div>

            <div className="form-group">
              <label>Elegí tu avatar</label>
              <div className="avatar-grid">
                {AVATARS.map(av => (<button key={av.id} className={`avatar-btn ${selectedAvatar === av.id ? 'selected' : ''}`} id={`avatar-${av.id}`} onClick={() => setSelectedAvatar(av.id)} title={av.label}>
            ...
            <div className="step-nav">
              ...
              <button className="btn-primary" id="step2-next" disabled={!username || !!usernameError} onClick={() => setStep(3)}>
                Continuar →
              </button>
            </div>
          </div>)}
        ...
        {/* STEP 3: Role & Confirm */}
        {step === 3 && (<div className="step-content" id="step-role">
            <div className="step-icon">🏆</div>
            <h2>Elegí tu rol</h2>

            <div className="role-cards">
              {ROLES.map(role => (<button key={role.id} className={`role-card ${selectedRole === role.id ? 'selected' : ''}`} id={`role-${role.id}`} onClick={() => setSelectedRole(role.id)}>
            ...
            <div className="step-nav">
              ...
              <button className="btn-primary btn-launch" id="create-account-btn" onClick={handleSubmit} disabled={isSubmitting}>
                {isSubmitting ? (<span className="loading-dots">Creando cuenta<span>...</span></span>) : ('🚀 Crear cuenta')}
              </button>
            </div>
          </div>)}
      </div>

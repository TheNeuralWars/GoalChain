 ...
 */
function VanillaMount({ gameId, mode = 'iframe', src, ctx, className, }) {
    ...
    useEffect(() => {
        ...
        import(/* @vite-ignore */ src)
            .then((mod) => {
            if (cancelled || !containerRef.current || !mod.init)
                return;
    ...
    return (<IframeGame gameId={gameId} url={url} className={className}/>);
}
/**
 ...
 */
function IframeGame({ gameId, url, className, }) {
    ...
    return (<div className={`gc-vanilla-host gc-vanilla-host--iframe ${className ?? ''}`}>
      <iframe key={gameId} src={url} title={`GoalChain ${gameId} game`} loading="lazy" sandbox="allow-scripts allow-same-origin allow-popups" allow="clipboard-write" className="gc-vanilla-iframe"/>
    </div>);
}

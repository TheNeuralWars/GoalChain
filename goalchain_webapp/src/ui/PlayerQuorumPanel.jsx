...
// Sub-components
// ─────────────────────────────────────────────────────────────────────────────
function GPBar({ current, max, color }) {
    const pct = max > 0 ? Math.min(100, (current / max) * 100) : 100;
    return (<div style={{ height: 8, borderRadius: 6, background: 'rgba(255,255,255,0.06)', overflow: 'hidden', position: 'relative' }}>
      <div style={{
...
function PlayerCard({ player, selected, onClick, signerIndex, }) {
    ...
    return (<div onClick={onClick} style={{
      ...
      {/* Signer badge */}
      {selected && signerIndex !== undefined && (<div style={{
      ...
      <div style={{ marginTop: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', fontSize: '0.58rem' }}>
          {[['ATK', player.stats.atk], ['DEF', player.stats.def], ['HYP', player.stats.hype]].map(([label, val]) => (<div key={label} style={{ flex: 1, textAlign: 'center', background: 'rgba(255,255,255,0.04)', borderRadius: '4px', padding: '3px 0' }}>
...
// Main Panel
// ─────────────────────────────────────────────────────────────────────────────
export function PlayerQuorumPanel({ initialOperation, onQuorumReached, onDismiss }) {
    ...
    const evaluate = useCallback(async (op, ids) => {
        if (!op.trim())
            return;
        setEvaluating(true);
        try {
    ...
    const handleOperationChange = (op) => {
        setOperation(op);
        if (selected.size > 0)
            evaluate(op, Array.from(selected));
        setSigned(false);
        setResult(null);
    };
    const handleSign = () => {
    ...
    const RISK_LEVEL_TRANS = {
        LOW: { en: 'LOW', es: 'BAJO' },
        MEDIUM: { en: 'MEDIUM', es: 'MEDIO' },
        HIGH: { en: 'HIGH', es: 'ALTO' },
        CRITICAL: { en: 'CRITICAL', es: 'CRÍTICO' },
    };
    const getQuorumLabel = (lbl) => {
        if (language !== 'es')
            return lbl;
        if (lbl.includes("No quorum required"))
            return "✅ No se requiere cuórum";
        if (lbl.includes("1 squad signer needed"))
            return "🟡 Se necesita 1 firmante del squad";
        if (lbl.includes("2 squad signers needed"))
            return "🟠 Se necesitan 2 firmantes del squad";
        if (lbl.includes("3 squad signers needed"))
            return "🔴 Se necesitan 3 firmantes del squad";
        return lbl;
    };
    const localizeMessage = (res) => {
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: '0.78rem', color: '#94a3b8', maxWidth: '440px' }}>
            {tText('High-risk agent operations require authorization from your Genesis Squad players. Select signers whose combined ', 'Las operaciones de agente de alto riesgo requieren autorización de tus jugadores del Genesis Squad. Selecciona firmantes cuyo ')}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '12px' }}>
          {DEMO_OPERATIONS.map(d => {
            ...
            return (<button key={opVal} onClick={() => handleOperationChange(opVal)} style={{
        ...
        <textarea value={operation} onChange={e => handleOperationChange(e.target.value)} placeholder={tText("Or type a custom operation: e.g. 'Stripe payment $300 for NVIDIA NIM API credits'", "O escribe una operación personalizada: ej. 'Pago Stripe $300 para créditos de API de NVIDIA NIM'")} rows={2} style={{
        ...
        {result && (<div style={{
            ...
            <div>
              ...
              <div style={{ fontSize: '0.62rem', color: '#64748b', marginTop: '2px' }}>
                {tText(`Requires ≥ ${result.gp_threshold} GP from ≥ ${result.min_players} player(s)`, `Requiere ≥ ${result.gp_threshold} GP de ≥ ${result.min_players} jugador(es)`)}
              </div>
            </div>
          </div>)}
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          ...
          {selected.size > 0 && (<div style={{ fontSize: '0.68rem', fontWeight: 800, color: '#fbbf24' }}>
              {tText(`${selected.size} selected`, `${selected.size} seleccionados`)} · {selectedGP.toFixed(1)} GP
            </div>)}
        </div>

        {loading ? (<div style={{ textAlign: 'center', padding: '20px', color: '#64748b', fontSize: '0.8rem' }}>{tText('Loading squad...', 'Cargando squad...')}</div>) : (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(155px, 1fr))', gap: '10px' }}>
            {squad.map((p, idx) => (<PlayerCard key={p.id} player={p} selected={selected.has(p.id)} onClick={() => handleSelect(p.id)} signerIndex={selected.has(p.id) ? Array.from(selected).indexOf(p.id) : undefined}/>))}
          </div>)}
      </div>

      {/* ── QUORUM METER ── */}
      {result && result.risk_level !== 'LOW' && (<div style={{
          ...
          {/* Active signers list */}
          {selectedList.length > 0 && (<div style={{ marginTop: '12px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {selectedList.map((p, idx) => (<div key={p.id} style={{
      ...
      {/* ── SIGN BUTTON ── */}
      {result && result.risk_level !== 'LOW' && (<button onClick={handleSign} disabled={!result.quorum_reached || signed} style={{
          ...
          {signed
                ? tText('✅ Quorum Confirmed — Agent Authorized to Execute', '✅ Cuórum Confirmado — Agente Autorizado para Ejecutar')
                : result.quorum_reached
                    ? tText(`⚡ Confirm Quorum — Authorize ${result.risk_level} Operation`, `⚡ Confirmar Cuórum — Autorizar Operación de Riesgo ${RISK_LEVEL_TRANS[result.risk_level]?.es || result.risk_level}`)
                    : tText(`❌ Quorum Insufficient — Need ${result.gp_deficit.toFixed(1)} more GP`, `❌ Cuórum Insuficiente — Faltan ${result.gp_deficit.toFixed(1)} GP`)}
        </button>)}
      ...
      {result && result.risk_level === 'LOW' && (<div style={{
          ...
          {tText('✅ LOW RISK — NemoClaw approved. No squad signature required. Agent executing directly.', '✅ RIESGO BAJO — NemoClaw aprobado. No se requiere firma del squad. El agente se ejecuta directamente.')}
        </div>)}
    </div>);
}

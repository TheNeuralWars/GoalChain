...
const VAULT_STRATEGIES = [
    ...
    },
    {
        ...
        mockLogs: [
            ...
            'Balanceando pools tras resolución de mercado por el oráculo GoalChain.'
        ]
    },
    {
        id: 'orchestrator',
...
export const SwarmVaults = () => {
    const [activeVaultId, setActiveVaultId] = useState('sentinel');
    const [walletGch, setWalletGch] = useState(2500);
    ...
    const handlePercentShortcut = (pct) => {
        const amount = Number((walletGch * pct).toFixed(2));
        setInputValue(amount.toString());
    };
    return (<div className="glass-card swarm-vaults-panel" style={{ textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div>
            ...
            return (<div key={strategy.id} onClick={() => setActiveVaultId(strategy.id)} className={`vault-selector-card ${isActive ? 'active' : ''}`} style={{
            ...
            gap: '1rem'
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <div className="stacked-bar-container">
                        {activeVault.allocations.map((alloc, idx) => (<div key={idx} className="stacked-bar-segment" style={{
                width: `${alloc.share}%`,
                ...
                boxShadow: `0 0 8px ${alloc.color}80`
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px 16px', marginTop: '4px' }}>
                        {activeVault.allocations.map((alloc, idx) => (<div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                    <div>
                        ...
                        <div style={{ display: 'flex', gap: '8px', position: 'relative' }}>
                            <input type="number" value={inputValue} onChange={(e) => setInputValue(e.target.value)} className="form-select" // Reutilizar estilos de inputs del terminal
     placeholder="0.00" style={{
            ...
            WebkitAppearance: 'none'
                            ...
                            <button onClick={handleDeposit} className="btn-neon-green" style={{ padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem' }}>
                                Delegar
                            </button>
                        </div>
                    </div>
                    ...
                    {/* Withdraw Button */}
                    {vaultBalances[activeVaultId] > 0 && (<button onClick={handleWithdraw} className="btn-outline-red" style={{ width: '100%', padding: '8px', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem', marginTop: '4px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    ...
                    <div className="terminal-console">
                        {consoleLogs.map((log, index) => (<div key={index} style={{ wordBreak: 'break-all', lineHeight: '1.3' }}>
                                {log}
                            </div>))}

...
/**
 * goalchain_program_environment.ts
 *
 * Typed seam for the network/cluster/program-ID/mint quartet used by
 * every GoalChain consumer (SDK inbound, API, Oracle, Webapp).
 *
 * BACKGROUND
 *
 ...
 *   GOALCHAIN_CLUSTER   - localnet | devnet | mainnet (default: devnet)
 ...
 *   1. goalchain_api/src/index.ts                    -> swap new Connection() for getConnection()
 *   2. goalchain_webapp/src/lib/goalchainClient.ts    -> use getConnection() and getProgramId()
 *   3. goalchain_oracle/src/internal scripts                   -> read RPC and PROGRAM_ID via this helper
...
function normaliseCluster(raw) {
    if (raw === 'localnet' || raw === 'devnet' || raw === 'mainnet')
        return raw;
    return 'devnet';
}
const CLUSTER = normaliseCluster(readEnv('GOALCHAIN_CLUSTER', 'devnet'));
const DEFAULT_RPC = CLUSTER === 'localnet'
    ? 'http://127.0.0.1:8899'
    : clusterApiUrl(CLUSTER === "mainnet" ? "mainnet-beta" : (CLUSTER === "devnet" ? "devnet" : "testnet"));
const RPC_URL = readEnv('RPC_URL', DEFAULT_RPC);
const PROGRAM_ID_BASE58 = readEnv('PROGRAM_ID', 'FbDhM4itBS2Cco7c7PbNvC98Fx7Y5HxqXS1JuXdNcBwg');
const GCH_TOKEN_MINT_BASE58 = readEnv('GCH_TOKEN_MINT', '');
 ...
 */
export function getGchTokenMint() {
...
export function getConnection(commitment = 'confirmed') {
    if (!_connection || _connection.commitment !== commitment) {
 ...
 */
export const GOALCHAIN_ENV = Object.freeze({
    cluster: CLUSTER,
    rpcUrl: RPC_URL,

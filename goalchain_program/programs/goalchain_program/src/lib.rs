...
pub mod goalchain_program {
    ...
    pub fn update_player_stats(
        ...
        let player = &mut ctx.accounts.parody_player;
        player.real_world_goals = player.real_world_goals.checked_add(new_goals).unwrap();
        player.real_world_assists = player.real_world_assists.checked_add(new_assists).unwrap();
        player.shot_power = player.shot_power.saturating_add(new_goals);
        Ok(())
    }
    ...
    pub fn resolve_wager(ctx: Context<ResolveWager>, _winner_is_a: bool) -> Result<()> {
        ...
        
        {
            ...
            total_payout = wager.amount.checked_mul(2).unwrap();
    ...
    pub fn place_bet(ctx: Context<PlaceBet>, prediction: MatchResult, amount: u64) -> Result<()> {
        ...
        require!(fixture.status != MatchStatus::Completed, GoalChainError::MatchAlreadyFinished);
        ...
        match prediction {
            MatchResult::TeamA => fixture.pool_a = fixture.pool_a.checked_add(amount).unwrap(),
            MatchResult::TeamB => fixture.pool_b = fixture.pool_b.checked_add(amount).unwrap(),
            MatchResult::Draw => fixture.pool_draw = fixture.pool_draw.checked_add(amount).unwrap(),
        }
    ...
    pub fn claim_bet_payout(ctx: Context<ClaimBetPayout>) -> Result<()> {
        ...
        let total_pool = fixture.pool_a + fixture.pool_b + fixture.pool_draw;
        
        let house_fee = total_pool.checked_mul(5).unwrap() / 100;
        let jackpot_fee = total_pool.checked_mul(2).unwrap() / 100;
        let treasury_fee = total_pool.checked_mul(3).unwrap() / 100;
        let net_pool = total_pool - house_fee - jackpot_fee - treasury_fee;
        ...
        // --- Transferencias de comisiones (Manual por brevedad en este paso) ---
        // TODO: Implementar CPI de transferencias a house/jackpot/treasury
...
pub struct InitializeFixture<'info> {
    #[account(mut)] pub oracle_authority: Signer<'info>,
    #[account(init, payer = oracle_authority, space = 8 + 64 + 64 + 64 + 8 + 8 + 8 + 1 + 2 + 8 + 1, seeds = [b"fixture", match_id.as_bytes()], bump)]
...
pub struct PlaceBet<'info> {
    ...
    #[account(init, payer = user, space = 8 + 32 + 32 + 8 + 1 + 8 + 1, seeds = [b"bet", user.key().as_ref(), fixture.key().as_ref()], bump)]
...
pub struct ClaimBetPayout<'info> {
    ...
    pub user_bet: Account<'info, UserBet>,
    #[account(mut)] pub user_token_account: InterfaceAccount<'info, TokenAccount>,
    #[account(mut)] pub fixture_vault: InterfaceAccount<'info, TokenAccount>,
    pub token_mint: InterfaceAccount<'info, Mint>,
    pub token_program: Interface<'info, TokenInterface>,
}
...
#[account]
pub struct Fixture {
    pub match_id: String, pub team_a: String, pub team_b: String,
    pub pool_a: u64, pub pool_b: u64, pub pool_draw: u64,
    pub status: MatchStatus, pub winner: Option<MatchResult>,
    pub start_timestamp: i64, pub bump: u8,
}

#[account]
pub struct UserBet { pub owner: Pubkey, pub fixture: Pubkey, pub amount: u64, pub prediction: MatchResult, pub bet_timestamp: i64, pub claimed: bool }

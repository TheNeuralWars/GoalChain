...
def _image_payload(image_path: Path) -> dict:
    """Inline the source still as a base64 data URL so the video model can
    animate THE still, instead of guessing from the prompt text alone.
    NOTE: the pipeline saves JPEG bytes under a .png extension, so the MIME
    type is sniffed from the magic bytes, never from the suffix."""
    import base64
    ...
    if raw[:8] == b"\x89PNG\r\n\x1a\n":
        ...
        raise RuntimeError(f"Unsupported image bytes for i2v source: {image_path}")
...
def generate_video_api(video_prompt: str, image_path: Path, dest: Path) -> Path:
    ...
    # TRUE image-to-video: POST /v1/videos/generations accepts an `image`
    # object (base64 data URL / public URL / Files API file_id). Uses the
    # i2v-capable model so the generated clip animates the storyboard still.
    if image_path is None or not Path(image_path).exists():
        raise RuntimeError(f"i2v requires an existing source image, got: {image_path}")

    payload_dict = {
        "model": "grok-imagine-video-1.5",
        "prompt": video_prompt,
        "image": _image_payload(Path(image_path)),
        "duration": 6,
        "resolution": "720p",
        "aspect_ratio": "16:9",
    }
    ...
    video_url = None
    # xAI queues 720p i2v renders under concurrency: a 6s clip can take 60-200s.
    # 200 x 3s = 10 min ceiling before we consider the render lost and fall back.
    for _ in range(200):
        time.sleep(3)

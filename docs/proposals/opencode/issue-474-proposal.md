# OA Proposal — Issue #474

## Title
[opencode] P1: Capture JitoSOL yield on treasury SOL holdings by staking via the existing `treasury_jito_ata` + Stake Pool CPI, creating a sustainable yield stream that feeds the buyback/burn loop and reduces net SOL emissions.
...
- Rollback: revert branch `exp/opencode-issue-474` and close draft PR.

/**
 * GoalChain Economy & Modifiers Simulator (V2.0)
...
class ModifiersSimulator {
    ...
    setupListeners() {
        ...
        if (this.licenseCheck) {
            this.licenseCheck.addEventListener('change', (e) => {
                ...
                this.logToConsole(`🏛️ [Licencia] Multiplicador de País ${this.hasCountryLicense ? 'ACTIVADO (+5% Yield, +10% Stats)' : 'desactivado'}.`);
                this.updateYieldAndUI();
            });
        }
        ...
        if (this.synergyCountSelect) {
            this.synergyCountSelect.addEventListener('change', (e) => {
                ...
                if (this.sameCountryCount >= 11) msg += "Sinergia de Full Squad (+15% Yield, +25% Stats) activa!";
                else if (this.sameCountryCount >= 5) msg += "Sinergia de 5 Jugadores (+12% Stats) activa.";
                else if (this.sameCountryCount >= 3) msg += "Sinergia de 3 Jugadores (+5% Stats) activa.";
        ...
        if (this.stadiumSelect) {
            this.stadiumSelect.addEventListener('change', (e) => {
                ...
                this.logToConsole(`🏟️ [Estadio] Clima de localía cambiado a: "${this.stadiumTheme.toUpperCase()}".`);
                this.updateYieldAndUI();
            });
        }
        ...
        document.querySelectorAll('input[name="simResult"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                this.logToConsole(`⚽ [Oráculo] Último resultado de selección resuelto como: ${this.lastMatchResult.toUpperCase()}.`);
                this.updateYieldAndUI();
            });
        });
        ...
        document.querySelectorAll('input[name="simStreak"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                let modeText = this.winStreak >= 4 ? "¡🔥 ON FIRE 🔥!" : `${this.winStreak} victorias`;
                this.logToConsole(`🔥 [Oráculo] Racha de victorias cambiada a: ${modeText}.`);
                this.updateYieldAndUI();
            });
        });
        ...
        if (this.curveWins || this.curveLosses || this.curveChampion) {
            ...
            if (this.curveLosses) this.curveLosses.addEventListener('input', updateCurve);
            if (this.curveChampion) this.curveChampion.addEventListener('change', updateCurve);
        }
    }
    ...
    loadPlayer(playerId) {
        if (!window.masterPlayers || window.masterPlayers.length === 0) {
            ...
            this.selectedPlayer = {
                id: playerId,
                name: playerId === 1 ? "Lionel Satoshi" : (playerId === 53 ? "Kylian M-bag-pé" : "Cristiano Holdaldo"),
                real_name: playerId === 1 ? "Lionel Messi" : (playerId === 53 ? "Kylian Mbappé" : "Cristiano Ronaldo"),
                country: playerId === 1 ? "Argentina" : (playerId === 53 ? "Francia" : "Portugal"),
                rarity: "mythic",
            ...
            if (found) {
                ...
                const yieldMap = { "mythic": 850, "legendary": 400, "epic": 190, "rare": 70, "common": 15 };
                this.selectedPlayer.base_yield_rate = yieldMap[this.selectedPlayer.rarity] || 70;
            }
        }
        ...
        this.resetSimState();
        this.logToConsole(`👤 [Jugador] Cargado: ${this.selectedPlayer.name} (${this.selectedPlayer.country}) | Rareza: ${this.selectedPlayer.rarity.toUpperCase()}`);
        
        // Actualizar datos del cromo
        if (this.cardName) this.cardName.innerText = this.selectedPlayer.name;
        if (this.cardRealName) this.cardRealName.innerText = this.selectedPlayer.real_name || "Verified Athlete";
        ...
        if (this.cardImg && typeof getPlayerImagePath === 'function') {
            ...
            const mappedName = this.selectedPlayer.name.toLowerCase().replace(/ /g, '_').replace(/[^a-z0-9_\-]/g, '');
            this.cardImg.src = `assets/img/nfts/${String(this.selectedPlayer.id).padStart(3, '0')}_${mappedName}.webp`;
        }
    ...
    updateStatsUI() {
        ...
        const baseSpeed = this.selectedPlayer.stats.speed || this.selectedPlayer.stats.spd || 75;
        const baseHype = this.selectedPlayer.stats.hype;

        if (this.atkStat) this.atkStat.innerText = Math.round((baseAtk + atkBoost) * statMultiplier);
        if (this.defStat) this.defStat.innerText = Math.round(baseDef * statMultiplier);
        if (this.spdStat) this.spdStat.innerText = Math.round((baseSpeed + speedBoost) * statMultiplier);
        if (this.hypStat) this.hypStat.innerText = Math.round((baseHype + hypeBoost) * statMultiplier);
    }

    updateYieldAndUI() {
        if (!this.selectedPlayer) return;

        // FÓRMULA DE YIELD DE GOALCHAIN V2.0
        ...
        else if (this.lastMatchResult === 'loss') mMatch = -0.10; // -10%

        // 2. Modificador de Racha (S_streak)
        let sStreak = 0;
        if (this.winStreak === 2) sStreak = 0.05;
        else if (this.winStreak === 3) sStreak = 0.10;
        else if (this.winStreak >= 4) sStreak = 0.20;
        ...
        const correctTheme = stadiumThemeMap[this.selectedPlayer.bg_type] || "desert";
        if (this.stadiumTheme === correctTheme) {
            ...
            if (document.getElementById('stadiumAdvantageBadge')) {
                document.getElementById('stadiumAdvantageBadge').style.display = 'inline-block';
            }
        } else {
            if (document.getElementById('stadiumAdvantageBadge')) {
    ...
    logToConsole(text) {
        ...
        else if (text.includes('burn') || text.includes('quema') || text.includes('Burns')) logItem.style.color = "#ff4d6a";
        else if (text.includes('ÉXITO') || text.includes('Fusión') || text.includes('fusionado')) logItem.style.color = "#14f195";
        else if (text.includes('Alquiler') || text.includes('Golden')) logItem.style.color = "#ffb703";
    ...
    feedSimPotion() {
        ...
        if (this.stamina >= this.maxStamina) {
            this.logToConsole("⚠️ [Sistema] La estamina del jugador ya está al máximo.");
            return;
        }
        ...
        if (typeof confetti === 'function') {
            confetti({ particleCount: 30, spread: 40, colors: ['#14f195', '#00ffcc'], origin: { y: 0.6 } });
        }
    ...
    toggleRentalListing() {
        ...
        if (this.rentalStatus === 'none') {
            ...
            this.logToConsole(`🏦 [Alquiler] Jugador listado para renta a un precio de ${this.rentalPrice} $GCH por partido.`);
            this.logToConsole(`[Solana Devnet] Instruction called: list_nft_rental. PDA Escrow registrado.`);
        } else {
            this.rentalStatus = 'none';
            this.logToConsole(`🏦 [Alquiler] Listado retirado del mercado. Jugador devuelto al Starting XI del dueño.`);
        }
    ...
    simulateBorrowerRent() {
        ...
        this.logToConsole(`🏦 [Alquiler] ¡Contrato firmado! El manager ${borrower} ha alquilado a ${this.selectedPlayer.name} por 1 partido.`);
        this.logToConsole(`💰 [Escrow] Recibidos ${this.rentalPrice} $GCH en tu balance (neto de tarifa de protocolo).`);
        this.logToConsole(`[Solana Devnet] Transfer completed: Inquilino -> Escrow -> Owner. Renter PDA active.`);
    ...
    triggerGoldenRecall() {
        ...
        if (this.userBalance < penalty) {
            alert(`No tienes suficiente $GCH para pagar la rescisión dorada (Costo de multa: ${penalty} $GCH).`);
            return;
        }
        ...
        this.logToConsole(`🏆 [Alquiler] ¡Cláusula de Rescisión Dorada ejecutada! Has retirado a tu jugador de forma inmediata del contrato.`);
        this.logToConsole(`🔥 [Burn Protocol] Penalización aplicada: pagados ${penalty} $GCH al inquilino ${this.currentBorrower} como resarcimiento.`);
        this.logToConsole(`[Solana Devnet] Instruction resolved: golden_recall. NFT retornado a tu Starting XI. TX: 3rFw...${Math.random().toString(36).substring(7).toUpperCase()}`);
        
        if (typeof confetti === 'function') {
            confetti({ particleCount: 50, spread: 60, colors: ['#ffb703', '#ffffff'] });
        }
    ...
    updateRentalUI() {
        ...
        if (this.rentalStatus === 'none') {
            this.rentalStatusText.innerHTML = '<span style="color:#a2a2b2;">Starting XI (Dueño)</span>';
    ...
    updateLiveBalanceWidget() {
        ...
        const widgetBalance = document.querySelector('.sidebar div[style*="Balance:"] span:last-child');
        if (widgetBalance) widgetBalance.innerText = `${this.userBalance.toLocaleString()} $GCH`;
        
        const simBalance = document.getElementById('simUserBalanceText');
        if (simBalance) simBalance.innerText = `${this.userBalance.toLocaleString()} $GCH`;
